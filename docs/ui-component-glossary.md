...
| nav wash, file nav wash, selection wash | File Nav wash | layer1-nav-wash.ts | reuses `.wash` in `#washes`; own marker `.nav-wash` | Band spanning the oldest→newest drawn event across the files currently selected in File Nav |
