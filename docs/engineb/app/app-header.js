...
// Split from app-router.ts (task 115). initializeHeader calls back into the router
// (renderRoute, setBreadcrumb, resetLastLoadedProject), so the dependency direction is
// header → router.
import { el } from "./app-dom.js";
import { reconcileServerBootId } from "./app-choices.js";
...
export async function pickFolderInto(target) {
...
export function prefillFileHistoryOverrideField(overrideDir) {
    const fileHistoryFields = document.getElementById("file-history-fields");
    const fileHistoryInput = document.getElementById("file-history-dir-input");
...
async function populateProjectsMenu(menu) {
...
export async function initializeHeader() {
    ...
    pathsPopover.addEventListener("click", (event) => {
        if (event.target.id !== "projects-dir-change") {
            event.stopPropagation();
        }
    });
    ...
    const computeFileHistoryDirToPost = () => {
