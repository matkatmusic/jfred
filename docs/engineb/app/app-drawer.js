...
import { peekCachedDocument } from "./app-fetch.js";
import { renderConversationView } from "./views/conversation.js";
import { computeFileRouteFocus } from "./views/details-model.js";
import { renderDetailsFileMode } from "./views/details-revision-view.js";
import { renderDiffVsBaseView } from "./views/diff-vs-base.js";
import { renderRawLinesView } from "./views/raw-lines.js";
import { activeDetailsContext } from "./views/timeline.js";
...
function renderFileRouteInRevisionView(target, segments) {
...
export async function renderSubRouteDrawer(project, segments) {
    ...
    if (segments[2] === "jsonl") {
        ...
        if (segments[4] === "lines") {
            ...
            renderContent = (content) => renderRawLinesView(content, project, jsonl);
        }
        else {
            headerText = `Conversation — ${jsonl}`;
            renderContent = (content) => renderConversationView(content, project, jsonl, segments[4] === "at" ? segments[5] : undefined);
        }
    }
        ...
        if (segments[4] === "vsbase") {
            ...
            renderContent = (content) => renderDiffVsBaseView(content, project, target, segments[5]);
        }
        else {
...
export function resetDetailsPane() {
