...
import { findRevisionForChangeId } from "./views/file-history-model.js";
import { computeRevisionLinkRoute, computeSnapshotHistoryAnchor, findBackupTimeForBlob, findJumpTarget, findTrackedBackupEntry, } from "./inspector-links.js";
// Typed view of app.ts's el helper, scoped to the attribute keys and children this file passes.
export const el = elUntyped;
...
function appendViewInRevisionsButton(pre, snapshotContext, anchor) {
...
function resolveStringTokenLink(pre, tokenClass, token, record, currentLine, maps, filesTouched, snapshotContext) {
    let jumpTarget;
    let revisionLink;
    try {
...
function appendJumpLinkSpan(pre, tokenClass, token, jumpTarget, showLine) {
    pre.append(el("span", {
        class: `${tokenClass} jump-link`,
...
function appendRevisionLinkSpan(pre, tokenClass, token, revisionLink, openRevision) {
    pre.append(el("span", {
        class: `${tokenClass} jump-link`,
...
function appendCollapsibleLongValue(pre, tokenClass, token) {
...
export function renderHighlightedJson(prettyText, record, currentLine, maps, showLine, filesTouched, openRevision, snapshotContext) {
