...
import { checkChangeIdIsBackupBlobName, findRevisionForChangeId } from "./file-history-model.js";
import { EDIT_EVENT_KIND, TOOL_CALL_NODE_KIND, } from "./timeline-types.js";
...
export function checkSnapshotIsGitBaseline(snapshot) {
...
// that time.
function computeEntryTimePaths(history) {
    const entryTimePaths = new Array(history.revisions.length);
...
// counts as surviving).
function recordHistoryRevisions(index, history, isRewound) {
...
function startNextPatchBlock(line, current, blocks) {
    if (current !== null) {
        blocks.push(current);
    }
...
// collapse per path.
function computeMergedChipKey(change) {
...
// lives there).
function mergeSnapshotFileChanges(snapshot, revisionIndex, seenChipKeys, changes) {
...
export function deriveNodeFileChanges(nodes, revisionIndex) {
