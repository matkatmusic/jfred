...
import { computeMatchCounterLabel, computeWrappedMatchIndex, findMatchesInTextNodeValues, } from "./details-find-model.js";
...
function collectRightPaneTextNodes() {
...
function updateFindCounterAndScroll() {
...
function stepDetailsFind(delta) {
    if (matchRanges.length === 0) {
        runDetailsFind(document.getElementById("details-find-input").value);
        return;
    }
...
export function resetDetailsFind() {
    matchRanges = [];
    currentMatchIndex = -1;
    if (CSS.highlights !== undefined) {
...
export function initDetailsFind() {
    const input = document.getElementById("details-find-input");
    const clearButton = document.getElementById("details-find-clear");
    input.oninput = () => {
