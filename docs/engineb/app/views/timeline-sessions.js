...
import { computeSessionShortLabel } from "./timeline-labels.js";
import { LINE_NODE_KIND } from "./timeline-line-nodes.js";
import { AGENT_TURN_NODE_KIND, COMMIT_NODE_KIND, SESSION_END_NODE_KIND, } from "./timeline-types.js";
...
export function findJsonlForSession(listing, sessionId) {
...
export function buildSessionsSidebarViewModel(nodes, listing) {
    const entries = [];
...
export function computeGraphLaneRuns(nodes) {
    const runs = [];
    let currentRun;
    nodes.forEach((node, index) => {
...
export function checkNodeIsAbandonedBranchTip(nodes, index) {
...
export function checkRowIsExpandable(node) {
...
export function findAdjacentFileTouchedIndex(nodes, fromIndex, direction) {
...
export function checkRowCarriesJsonRecordButton(node) {
    if (node.kind === COMMIT_NODE_KIND) {
        return false;
    }
...
export function findAdjacentRowIndex(nodes, fromIndex, direction) {
...
export function checkTimelineNeedsProgressOverlay(rowCount) {
    return rowCount >= LARGE_TIMELINE_ROW_COUNT;
}
// The overlay's text line, e.g. "Building timeline… 250 / 1200 rows".
export function computeTimelineBuildProgressLabel(rowsBuilt, totalRows) {
...
export function computeTimelineProgressFraction(rowsBuilt, totalRows) {
    if (totalRows === 0) {
        return 1;
    }
