...
import { buildFileHistoryViewModel } from "./file-history-model.js";
import { splitPatchByFile } from "./timeline-changes.js";
import { computeRevisionDiffFallbackText } from "./timeline-labels.js";
import { computeRangeSummary } from "./timeline-picks.js";
import { RevisionViewMode, buildRevisionCards, checkCardRunIsContiguous, checkChangeIdIsGitBaseline, computeFocusedCardIndex, computeOwningNodeIndexes, fullContentsIsOn, } from "./details-model.js";
import { showGitBaselineInDetails } from "./details-baseline.js";
import { fetchRevisionDiffBlocks, setDetailsHeader, showContentInDetails, showDiffInDetails, showTextInDetails, showUnrecoverableInDetails, } from "./details-diff.js";
import { buildMissingRevisionCard, buildRevisionActionsElement, buildRevisionHeadElement, } from "./details-revision-cards.js";
export function renderDetailsFileMode(target, context, focus) {
    revealDetailsPane();
    ...
    const showCardDiff = async (card, index) => {
    ...
    const showCardInMode = (card, index, mode) => {
    ...
    const showRangeDiff = async () => {
        ...
        const block = splitPatchByFile(patchText).find((entry) => target === entry.path || target.endsWith(`/${entry.path}`));
    ...
    const buildActionButton = (text, onActivate) => el("button", {
        text,
        onclick: ((event) => {
