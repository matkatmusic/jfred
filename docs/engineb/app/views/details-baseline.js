...
import { extractGitBaseCommitHash } from "./timeline-changes.js";
import { buildFileHistoryViewModel, findRevisionForChangeId, } from "./file-history-model.js";
export function showGitBaselineInDetails(historyDocument, path, changeId) {
    ...
    body.prepend(el("div", { class: "recon-banner" }, [
        el("span", { class: "warn", text: "Git-derived baseline" }),
        ` — content seeded from base commit ${extractGitBaseCommitHash(changeId)}`,
    ]), el("pre", { class: "diff-text", text: "(this revision's bytes come from the configured git base commit, not from any session record)" }), el("div", { class: "pane-title", text: "— baseline content —" }));
}
