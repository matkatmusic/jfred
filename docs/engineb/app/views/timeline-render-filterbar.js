...
import { clearBaselineChoice, getBaselineChoice } from "../app-choices.js";
import { dropProjectDocuments } from "../app-fetch.js";
import { renderRoute } from "../app-router.js";
import { computeMatchCounterLabel, computeWrappedMatchIndex } from "./details-find-model.js";
import { checkAllLinesIsOn, toggleAllLinesSetting } from "./timeline-line-nodes.js";
import { TIMELINE_FILTER_BUTTONS, TIMELINE_FILTER_MODES, checkNodePassesFilters, computeMatchingNodeIndexes, computeSessionTitleByNodeIndex, } from "./timeline-filter-model.js";
...
function applyTimelineFilters(context, mode, term, sessionTitleByNodeIndex) {
...
export function renderTimelineFilterBar(context, bar) {
    let activeMode = TIMELINE_FILTER_MODES.all;
    let searchTerm = "";
    const searchInput = el("input", { class: "timeline-search", type: "search", placeholder: "🔍 search" });
    ...
    let matchNodeIndexes = [];
    let currentMatchPosition = -1;
    let currentMatchRow = null;
    const countLabel = el("span", { class: "search-count" });
    const prevButton = el("button", { class: "search-step", text: "▲", title: "Previous result" });
    const nextButton = el("button", { class: "search-step", text: "▼", title: "Next result" });
    const clearButton = el("button", { class: "search-clear", text: "✕", title: "Clear search" });
    ...
    for (const [mode, label] of TIMELINE_FILTER_BUTTONS) {
        const button = el("button", { class: "toolbar-btn", text: label });
    ...
    const allLinesButton = document.getElementById("all-lines-btn");
    ...
    const reaskBaselineButton = document.getElementById("reask-baseline-btn");
