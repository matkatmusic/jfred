...
export function truncateToolCallSummary(summary) {
...
function checkAgentTurnOwnsRawLine(node, rawLineText) {
    ...
    return node.snapshots.some((snapshot) => snapshot.changeIds.some((changeId) => rawLineText.includes(changeId)));
}
...
function checkToolCallOwnsRawLineByRecord(node, rawLineText) {
...
function checkToolCallOwnsRawLineByToolUseId(node, rawLineText) {
    if (node.kind !== TOOL_CALL_NODE_KIND) {
        return false;
    }
...
// (render as a diff).
export function computeRevisionDiffFallbackText(block, change) {
    if (block === undefined) {
...
export function computeSessionShortLabel(sessionId) {
...
export function formatRowTimestamp(when) {
...
export function computeSessionStartLabel(sessionTitles, sessionId) {
...
export function findSessionStartIndexes(nodes) {
    const starts = [];
...
// them).
export function computeRolePillLabel(node) {
    if (node.kind === USER_TURN_NODE_KIND)
        return "User";
    if (node.kind === TOOL_CALL_NODE_KIND)
        return "Tool";
    ...
    if (node.isGitBaseline === true)
        return GIT_BASELINE_ROLE_PILL_LABEL;
...
export function computeRolePillClass(label) {
...
export function computeRowSummaryText(node) {
