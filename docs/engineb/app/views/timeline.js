...
import { fetchDocument, fetchJson, fetchRawRecords } from "../app-fetch.js";
import { renderConsentDialog } from "../app-consent.js";
import { renderBaselineQuestionDialog } from "../app-baseline-question.js";
import { getModeChoice } from "../app-choices.js";
import { renderModeSelectionView } from "../app-mode-select.js";
import { showLoadingProgress } from "../app-progress.js";
import { openInspectorPane } from "../inspector.js";
import { renderDetailsFileMode } from "./details-revision-view.js";
import { renderForkSidebar } from "./sidebar.js";
import { buildCoverageSegmentsByTarget, renderReconstructionBanner } from "./reconstruction-render.js";
import { buildFileTree, buildFilesSidebarViewModel, } from "./timeline-file-tree.js";
import { findTimelineNodeIndexForRawLine } from "./timeline-labels.js";
import { checkAllLinesIsOn } from "./timeline-line-nodes.js";
import { buildTurnTimelineViewModel } from "./timeline-nodes.js";
import { clearActiveChip, markChipActive } from "./timeline-render-chips.js";
import { findTranscriptLineForChangeId, openNodeInspector, openTranscriptInspectorSynced, resolveChipLineLocations, resolveLineLabels, } from "./timeline-render-inspectors.js";
import { renderTimelineFilterBar } from "./timeline-render-filterbar.js";
import { buildTimelineRows } from "./timeline-render-rows.js";
import { fetchRangePatch, renderSelectbarButtons, updateSelectbar } from "./timeline-render-selectbar.js";
import { flashRowElement, jumpToTimelineRow, selectTimelineRow, wireFileNavButtons, wireToggleAllButton, } from "./timeline-render-selection.js";
import { buildSessionsSidebarViewModel, computeGraphLaneRuns, SESSION_LANE_VARIABLES, waitForNextAnimationFrame, } from "./timeline-sessions.js";
...
export let activeDetailsContext;
function selectTimelineRowAndFlash(context, nodeIndex) {
...
async function openRecordInspectorForChangeId(context, changeId) {
    ...
    if (located === undefined) {
        openInspectorPane().append(el("div", { class: "muted", text: "no transcript line for this revision (synthetic change id)" }));
        return;
    }
    openTranscriptInspectorSynced(context, located);
}
function renderFileDetails(target, detailsContext) {
...
export async function renderTimelineView(container, project, anchorJsonl, anchorLine) {
    ...
    const context = {
        ...
        barText,
        ruleHint,
        selectTimelineRow: (nodeIndex) => selectTimelineRow(context, nodeIndex),
        jumpToTimelineRow: (nodeIndex) => jumpToTimelineRow(context, nodeIndex),
        openNodeInspector: (nodeIndex) => openNodeInspector(context, nodeIndex),
        clearActiveChip: () => clearActiveChip(context),
        markChipActive: (chipElement) => markChipActive(context, chipElement),
        ...
        detailsContext: {
            ...
            nodes,
            openNodeInspector: (nodeIndex) => openNodeInspector(context, nodeIndex),
            selectTimelineRow: (nodeIndex) => selectTimelineRowAndFlash(context, nodeIndex),
            ...
            openRecordForChangeId: (changeId) => void openRecordInspectorForChangeId(context, changeId),
            fetchRangePatch: (fromStep, toStep) => fetchRangePatch(context, fromStep, toStep),
        },
    };
    ...
    renderForkSidebar(document.getElementById("drawer"), buildSessionsSidebarViewModel(nodes, listing), buildFileTree(buildFilesSidebarViewModel(reconstructionDocument)), {
        onSessionClick: context.jumpToTimelineRow,
        onFileClick: (target) => renderFileDetails(target, context.detailsContext),
    ...
    if (anchorJsonl !== undefined && anchorLine === undefined) {
        const firstNodeIndex = nodes.findIndex((candidate) => candidate.sessionId !== undefined && anchorJsonl.startsWith(candidate.sessionId));
        if (firstNodeIndex >= 0) {
