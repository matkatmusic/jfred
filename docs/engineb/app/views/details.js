...
import { revealDetailsPane } from "../inspector.js";
import { deriveCommitChangedFiles } from "./timeline-commit-files.js";
import { applyRenameBadgeLabels } from "./timeline-file-badges.js";
import { buildFileTree, buildFilesSidebarViewModel, } from "./timeline-file-tree.js";
import { checkChangeIdIsGitBaseline, computeDetailsHeaderText, fullContentsIsOn } from "./details-model.js";
import { showGitBaselineInDetails } from "./details-baseline.js";
import { clearRightPaneBody, fetchRevisionDiffBlocks, hideDiffModeToggle, setDetailsHeader, setRightPaneLabel, showRevisionDiffInDetails, } from "./details-diff.js";
import { renderFileTreeNode } from "./sidebar.js";
...
function buildTouchedFileEntries(changes, wireDocument) {
    const sidebarEntriesByTarget = new Map(buildFilesSidebarViewModel(wireDocument).map((entry) => [entry.target, entry]));
...
function showClickedFileDiff(target, changeByPath, showFileDiff) {
...
function appendFileList(left, changes, context) {
    ...
    const showFileDiff = async (change) => {
    ...
    return [...left.querySelectorAll(".file-item")];
}
...
export function renderDetailsMessageMode(node, nodeIndex, context) {
    revealDetailsPane();
...
export function renderDetailsCommitMode(node, nodeIndex, context) {
    revealDetailsPane();
    setDetailsHeader(computeDetailsHeaderText(node, { index: nodeIndex, total: context.nodes.length }));
    const left = document.getElementById("details-left");
