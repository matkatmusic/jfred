...
import { computeRolePillClass, computeRolePillLabel, computeRowSummaryText, computeSessionShortLabel, findSessionStartIndexes, formatRowTimestamp, } from "./timeline-labels.js";
import { appendGapRows, computeGapRowBuckets } from "./reconstruction-render.js";
import { appendCommitCells, appendExpandedBubble, appendExpansionTriangle, appendJsonRecordButton, appendSessionStartMarker, buildGraphCell, computeRoleClass, } from "./timeline-render-row-cells.js";
import { buildPickCell } from "./timeline-render-selectbar.js";
import { checkNodeIsAbandonedBranchTip, checkRowCarriesJsonRecordButton, checkRowIsExpandable, checkTimelineNeedsProgressOverlay, computeTimelineBuildProgressLabel, computeTimelineProgressFraction, ORPHAN_LANE_COLOR, TIMELINE_BUILD_BATCH_SIZE, waitForNextAnimationFrame, } from "./timeline-sessions.js";
import { COMMIT_NODE_KIND, } from "./timeline-types.js";
// ── rows: one .tl-row per node (mockup renderTimeline) ──
export async function buildTimelineRows(context, container) {
    ...
    const sessionStartsByIndex = new Map(findSessionStartIndexes(context.nodes).map((start) => [start.nodeIndex, start.sessionId]));
