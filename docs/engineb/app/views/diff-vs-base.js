...
import { el as elFromApp } from "../app-dom.js";
import { getBaselineChoice, getConsentChoice } from "../app-choices.js";
import { fetchDocument, fetchText, } from "../app-fetch.js";
...
import { computeInlineRows, computeSplitRows, DIFF_MODE_STORAGE_KEY, DiffDisplayMode, resolveInitialDiffDisplayMode, SplitRowKind, } from "./diff-vs-base-model.js";
import { buildFileHistoryViewModel, computeAnchoredRevisionIndex } from "./file-history-model.js";
...
function renderInlineDiffLines(pane, diffText) {
    ...
    for (const row of computeInlineRows(diffText)) {
        ...
        grid.append(el("div", { class: "diff-line-num", text: row.oldLineNumber === undefined ? "" : String(row.oldLineNumber) }), el("div", { class: "diff-line-num", text: row.newLineNumber === undefined ? "" : String(row.newLineNumber) }), el("div", { class: row.lineClass, text: row.text }));
    }
    pane.append(grid);
}
...
function appendSplitCell(grid, cell) {
    if (cell === undefined) {
        grid.append(el("div", { class: "diff-line-num" }), el("div", {}));
        return;
    }
    grid.append(el("div", { class: "diff-line-num", text: cell.lineNumber === undefined ? "" : String(cell.lineNumber) }), el("div", { class: cell.lineClass, text: cell.text }));
}
