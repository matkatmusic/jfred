...
function pushPendingPairRow(rows, pendingDeletions, pendingAdditions, pairIndex) {
    rows.push({
...
function assignContextLineNumbers(row, oldLineCounter, newLineCounter) {
    if (oldLineCounter !== undefined) {
...
export function computeInlineRows(diffText) {
    const rows = [];
    let insideHunk = false;
    let oldLineCounter = undefined;
    let newLineCounter = undefined;
    for (const line of diffText.split("\n")) {
        ...
        const row = { text: line, lineClass: computeFullRowLineClass(line) };
