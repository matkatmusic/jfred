...
import { DIFF_MODE_STORAGE_KEY, DiffDisplayMode, resolveInitialDiffDisplayMode, } from "./diff-vs-base-model.js";
import { COMMIT_NODE_KIND, } from "./timeline-types.js";
import { computeAnchoredRevisionIndex } from "./file-history-model.js";
import { formatRowTimestamp } from "./timeline-labels.js";
...
export function checkChangeIdIsGitBaseline(changeId) {
...
export var RevisionViewMode;
(function (RevisionViewMode) {
    RevisionViewMode["diff"] = "diff";
    RevisionViewMode["content"] = "content";
    RevisionViewMode["record"] = "record";
...
export function computeFileRouteFocus(filesTouched, target, anchorRev) {
...
export function computeDetailsHeaderText(node, position) {
...
export function buildRevisionCards(history) {
...
export function computeFocusedCardIndex(cards, focus) {
    if (focus === undefined) {
        return 0;
    }
...
export function checkCardRunIsContiguous(selectedIndexes) {
...
export function computeOwningNodeIndexes(cards, nodes, selectedIndexes) {
    const ownerIndexes = [];
    for (const cardIndex of selectedIndexes) {
        ...
        const ownerIndex = nodes.findIndex((candidate) => (candidate.fileChanges ?? []).some((change) => change.changeId === card.changeId));
        if (ownerIndex < 0) {
            continue;
        }
...
export function mapStoredDiffModeToToggle(stored) {
...
export function writeStoredDiffMode(label) {
    if (typeof window === "undefined") {
        return;
    }
