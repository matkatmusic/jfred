...
function buildFileRow(file, selectAsBoundFile) {
    const row = el("div", {
        class: "mode-file-row",
...
export async function renderModeSelectionView(container, project) {
    container.replaceChildren(el("div", { class: "muted", text: "scanning touched files…" }));
    const { files } = await fetchJson(`/api/prescan?project=${encodeURIComponent(project)}`);
    let boundFilePath;
    let selectedRow;
    const fullRadio = el("input", { type: "radio", name: "recon-mode", checked: "" });
    const boundedRadio = el("input", { type: "radio", name: "recon-mode" });
    const nthInput = el("input", { type: "number", min: "1", value: "1", style: "width:64px" });
    const selectAsBoundFile = (path, row) => {
        boundFilePath = path;
