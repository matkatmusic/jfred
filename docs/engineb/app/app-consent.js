...
import { storeConsentChoice } from "./app-choices.js";
import { checkConsentScriptOverflowsPreview, clampConsentSelectionStep, ConsentBlockKind, CONSENT_SCRIPT_LANGUAGE_PATH, findDefaultConsentSelectionIndex, formatConsentSourceToken, groupConsentScriptsIntoBlocks, splitInlineInterpreterCode, } from "./app-consent-model.js";
...
function buildConsentScriptRow(script) {
    const pre = el("pre");
...
function toggleReadOnlyBlocksVisibility(box) {
    const detailsBlocks = [...box.querySelectorAll("details.consent-readonly-block")];
    const shouldOpen = detailsBlocks.some((block) => !block.open);
    for (const block of detailsBlocks)
        block.open = shouldOpen;
}
function appendReadOnlyScriptsBlock(box, scripts) {
...
function updateRowExpandButtonLabel(pre, shouldExpand) {
...
export function renderConsentDialog(container, project, scripts) {
    ...
    const runButton = el("button", { class: "toolbar-btn consent-run", text: "Run scripts for this reconstruction", onclick: () => decide("1") });
    const continueButton = el("button", { class: "toolbar-btn", text: "Continue without running", onclick: () => decide("0") });
    const jumpToTopButton = el("button", { class: "toolbar-btn consent-jump-top", text: "Jump to top", onclick: () => container.scrollTo({ top: 0, behavior: "smooth" }) });
    const prevButton = el("button", { class: "toolbar-btn", text: "< Prev" });
    const nextButton = el("button", { class: "toolbar-btn", text: "Next >" });
    const navCounter = el("span", { class: "muted consent-nav-counter" });
    const expandAllButton = el("button", { class: "toolbar-btn", text: "Expand All" });
    ...
    const allConsentRows = () => [...box.querySelectorAll(".consent-script")];
    ...
    let selectedRow = defaultIndex === undefined ? undefined : allConsentRows()[defaultIndex];
    ...
    const navigateConsentScript = (delta) => {
        const visibleRows = collectVisibleConsentRows();
    ...
    const collectExpandables = () => ({
        previews: [...box.querySelectorAll(".consent-script pre")]
            .filter((pre) => checkConsentScriptOverflowsPreview(pre.textContent ?? "")),
        readOnlyBlocks: [...box.querySelectorAll("details.consent-readonly-block")],
    });
    ...
    toggleAllButton.onclick = () => {
        ...
        for (const pre of previews)
            pre.classList.toggle("expanded", shouldExpand);
        for (const block of readOnlyBlocks)
            block.open = shouldExpand;
        for (const pre of previews) {
    ...
    document.getElementById("toggle-all").hidden = true;
