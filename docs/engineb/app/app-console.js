...
import { parseRouteSegments, routeToTimeline } from "./app-routes.js";
export let progressTerminal = null;
...
function navigateToSourceLink(sourceLink) {
...
function provideConsoleSourceLinks(bufferLineNumber, callback) {
...
export function ensureProgressTerminal() {
    if (progressTerminal !== null)
        return;
...
function fitProgressColumns() {
    ...
    if (dimensions?.cols && dimensions?.rows)
        progressTerminal.resize(dimensions.cols, dimensions.rows);
}
...
function findLastNonEmptyConsoleLine() {
    const buffer = progressTerminal.buffer.active;
...
export function collapseProgressConsole() {
...
export function expandProgressConsole() {
