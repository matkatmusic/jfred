// Task 197 (spec S1): instant axis placement — git committer seconds widened ×1000 onto the shared UTC-ms axis, with the Q7/Q9 content-order tiebreak for same-second commit-vs-JSONL placements.
...
test("test_widenEpochSecondsToInstant_multiplies_by_1000", () => {
    // Scenario: git committer time arrives in whole epoch seconds; the shared axis is UTC ms.  Steps: widen a known committer second.
...
test("test_same_second_equal_content_places_commit_after_jsonl", () => {
    // Scenario: a commit and a JSONL row share a second and hold IDENTICAL content — the commit blob snapshotted the state the JSONL row produced, so the commit sorts after it.  Steps: a widened (git) placement at the second boundary and an ms (JSONL) placement inside it.
...
test("test_same_second_differing_content_places_commit_before_jsonl", () => {
    // Scenario: same-second pair with DIFFERING content — had the commit happened after the JSONL change, its blob would hold that content, so the commit sorts before it.  Steps: a widened placement whose content is the older state.
...
test("test_same_second_unknown_content_falls_back_to_ms_order", () => {
    // Scenario: the tiebreak needs BOTH contents; a byteless side degrades to plain ms order.  Steps: a widened placement with no content at the second boundary.
...
test("test_different_seconds_order_by_ms", () => {
    // Scenario: the content tiebreak applies only WITHIN a second — across seconds the widened flag is irrelevant and ms order wins even when contents are equal.  Steps: a widened placement one second after an ms placement, identical content.
...
test("test_same_precision_same_ms_compare_equal_for_stable_source_order", () => {
    // Scenario: two JSONL placements at the exact same ms — the comparator reports equality so a stable sort preserves source (line) order, which IS content order within a session.  Steps: two ms placements at one instant.
