// Task 145: parallel tool calls split ONE assistant API response into sibling JSONL records sharing message.id, and each tool_result parents onto ITS OWN tool_use sibling — so the surviving ancestor walk reaches only one sibling's chain and the others dead-end (s85's `Write two.py`). Those dead ends are the same live turn, not a rewind; the trunk absorbs them. A real rewound branch (different message.id, prompt-first diverging record) must stay excluded.
...
// One tool_use record of a (possibly split) assistant API response: messageId is the response's message.id, shared across every sibling record the split produced.
...
// The s85 fork shape: prompt P -> tool_use A1 (msg M1) -> sibling tool_use A2 (msg M1) with its result R2 dead-ending, while the trunk continues from A1 via R1 -> A3 (msg M2, the tip).
...
test("test_select_live_branch_keeps_parallel_tool_call_sibling_write", () => {
    // Scenario: the sibling tool_use A2 shares trunk record A1's message.id — same API response, same live turn — so the live branch keeps it despite the parentUuid dead end.
...
test("test_select_live_branch_keeps_the_sibling_tool_result", () => {
    // Scenario: R2 is a tool_result-only user record whose parent (A2) is absorbed — the closure keeps the result too, so extraction sees the complete Write.
    const kept = selectLiveBranch(buildParallelToolCallRecords());
    const uuids = kept.filter((record) => record.uuid).map((record) => record.uuid!.toString());
...
test("test_absorb_does_not_swallow_a_tool_result_leading_to_real_content", () => {
    // Scenario: a hook-rewrite fork gives tool_use A1 two tool_result children; the dead one (R2) is followed by an assistant TEXT reply — a real rewound exchange (the orphans suite dims it), NOT parallel-tool-call plumbing. The absorb must leave R2 excluded.
...
test("test_absorb_does_not_swallow_a_real_rewound_branch", () => {
    // Scenario: a genuine rewind forks at P into an assistant turn from a DIFFERENT API response (msg M9) followed by an abandoned user prompt; neither absorb rule matches, so the rewound branch stays excluded from the live selection.
    const records: TranscriptRecord[] = [
        rec(RecordType.user, "P", null),
        assistantToolUse("A1", "P", "M1", "tool-one"),
