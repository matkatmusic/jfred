// Spec S6 (task 177) server side: a project entry declaring `sources` serves the union of every source's project JSONLs; entries without `sources` keep the single-dir scan (pinned).
...
function makeTwoTreeServerFixture(): { jsonlA: string; jsonlB: string; treeAProjects: string; treeBProjects: string } {
    ...
    // resolveProjectFile realpaths at the trust boundary, so expectations must too (the macOS temp dir is a /var -> /private/var symlink).
...
test("test_resolve_jsonl_paths_unions_configured_sources", () => {
    // Scenario (spec S6): with a session paths entry declaring two sources, the unified project view serves the JSONLs of BOTH source trees.  Step: two trees, one jsonl each; the session entry lists both projects roots.
...
test("test_resolve_jsonl_paths_without_sources_scans_single_dir", () => {
    // Scenario (legacy pin): without a `sources` entry, only the active projects dir's project files are served — the pre-S6 behavior, byte for byte.
    const fixture = makeTwoTreeServerFixture();
    const resolved = resolveJsonlPaths(PROJECT_NAME, null).map((path) => path.toString());
