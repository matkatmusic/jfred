// Task 281: the File Nav's own search box narrows the TREE as the user types, and its clear button puts every file back. It is not the find-bubble box: nothing here scrolls the timeline.
//
// The page-facing renderLayer1FileNav is the subject rather than renderFileNavInto, because the wiring IS the feature — the filter itself is one `includes` call, and a box nobody listened to would pass every assertion written against the renderer alone.
...
// One of each kind the endpoint returns, sharing the `src/` prefix buildFileTree strips. `gone.ts` sits a directory deeper so a query can match across a folder boundary.
const NAV_VIEW = {
...
// Boot the pane against the page's own host elements. Folder clicks belong to tests/layer1-folder-filter.test.ts, so the selection callback is a no-op here.
...
// Every leaf's full path — `title` is where the leaf renderer puts it (the visible text is the basename only), and a repo-only path carries a " (deleted)" suffix there.
...
test("test_typing_in_the_file_nav_search_box_leaves_only_matching_files", async () => {
    // Scenario (task 281): the tree filters as text is entered.  Steps: boot the nav over all three paths.
    await bootFileNav();
...
test("test_the_file_nav_search_clear_button_restores_every_file", async () => {
    // Scenario (task 281): the clear button empties the box AND resets the tree.  Steps: boot the nav and filter it down to one file.
    await bootFileNav();
