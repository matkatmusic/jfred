// Tolerant-mode (viewer) parsing: unparseable lines are skipped and captured as SkippedLines
// instead of aborting the load. Split from loadTranscript.test.ts (250-line cap).
...
// Write a throwaway .jsonl with the given lines, in a fresh temp dir (a fresh path is also a
// fresh transcript-set stamp, so the loadProjectRecords cache test starts cold).
...
test("test_loadTranscript_tolerant_skips_unknown_record_type_and_records_reason", () => {
    // Scenario: the viewer's tolerant load survives a record type outside the known vocabulary —
    // the line is skipped and captured with its reason and (hydrated) timestamp, not thrown.
    // Steps: load a file with one valid record and one unknown-type line, tolerantly.
...
test("test_loadTranscript_tolerant_skips_malformed_json_line", () => {
    // Scenario: a line that is not JSON at all is skipped with a "malformed JSON" reason; no
    // timestamp can be read from it.
...
test("test_loadTranscript_strict_still_throws_on_unknown_record_type", () => {
    // Scenario: strict mode (the CLI and the tests) keeps the fog-of-war guard — the same
    // unknown-type fixture that tolerant mode skips still throws.
    const jsonlPath = writeTempJsonl([VALID_MODE_LINE, UNKNOWN_TYPE_LINE]);
...
test("test_loadProjectRecords_caches_skipped_lines_with_records", () => {
    // Scenario: skips are discovered at parse time, so the parsed-records cache must carry them —
    // a warm rebuild reports the same gaps as the cold load.
    // Steps: load the same fixture set twice (cold parse, then cache hit).
