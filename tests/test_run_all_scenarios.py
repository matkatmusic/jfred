"""Tests for the copy-on-completion helpers in run-all-scenarios.py."""
...
mod = importlib.import_module("run-all-scenarios")
extractTmpdirFromResultText = mod.extractTmpdirFromResultText
extractJsonlPathFromResultText = mod.extractJsonlPathFromResultText
copyScenarioOutputsToExecutedDir = mod.copyScenarioOutputsToExecutedDir
copyScenarioJsonlToExecutedDir = mod.copyScenarioJsonlToExecutedDir
captureCompletedScenario = mod.captureCompletedScenario
...
def test_extractJsonlPathFromResultText_returnsTheJsonlPath():
    # The result text has a "result: {json}" line with a jsonl_path key.
    # extractJsonlPathFromResultText should parse the JSON and return it.
    jsonl_path = "/Users/someone/.claude/projects/abc/def-1234.jsonl"
    text = (
        "scenario: s1\n"
        "tmpdir: /private/tmp/scenario-abc123\n"
        f'result: {{"completed": true, "jsonl_path": "{jsonl_path}"}}\n'
    )

    result = extractJsonlPathFromResultText(text)
