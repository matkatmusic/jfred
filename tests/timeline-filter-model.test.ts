// Task 114: timeline filter predicate tests, one behavior per truth-table row.
...
test("test_all_mode_matches_every_node_kind", () => {
    // Scenario: "All" is the default mode and hides nothing.  Steps: run the predicate over every fixture shape; each must match.
...
test("test_conversation_mode_matches_user_and_agent_turns", () => {
    // Scenario: "Conversation" keeps the message rows — user prompts and agent replies.  Steps: a user turn matches; an agent turn matches.
...
test("test_conversation_mode_rejects_tool_call_and_commit_nodes", () => {
    // Scenario: "Conversation" hides non-message rows.  Steps: a tool-call row is hidden; a commit row is hidden.
...
test("test_tools_mode_matches_only_tool_call_nodes", () => {
    // Scenario: "Tools" keeps tool-call rows (each row carries its call AND its result summary).  Steps: both tool-call flavors match; turns and commits are hidden.
...
test("test_scripts_mode_matches_tool_call_with_script_run", () => {
    // Scenario: "Scripts" keeps tool-call rows that executed a script, file-modifying or not.  Steps: a script run with changed files matches; a read-only script run matches too.
...
test("test_scripts_mode_matches_agent_turn_with_script_execution_file_change", () => {
    // Scenario: "Scripts" keeps agent turns whose file chips carry a script-made revision — the exact rule the "Script" role pill uses (computeRolePillLabel).  Steps: an agent turn with a script-execution file change matches.
...
test("test_scripts_mode_rejects_plain_tool_call_and_plain_agent_turn", () => {
    // Scenario: "Scripts" hides rows with no script involvement.  Steps: a scriptless tool call is hidden; a plain agent turn is hidden; an agent turn whose only file change is a hand edit is hidden.
...
test("test_files_mode_matches_node_with_file_changes", () => {
    // Scenario: "Files" keeps rows carrying file-change chips, whatever made the change.  Steps: an agent turn with an edit chip matches; one with a script-made chip matches.
...
test("test_files_mode_matches_tool_call_whose_script_run_changed_files", () => {
    // Scenario: "Files" keeps script-run rows only when the sandbox proved files changed (scriptRun.changedPaths is [] on read-only/declined runs).  Steps: a file-modifying script run matches; a read-only script run is hidden.
...
test("test_files_mode_rejects_nodes_without_file_changes", () => {
    // Scenario: "Files" hides rows that touched no files.  Steps: chipless turns, a plain tool call, and a commit are all hidden.
...
test("test_git_mode_matches_only_commit_nodes", () => {
    // Scenario: "Git" keeps the commit hard-stop rows and nothing else.  Steps: a commit matches; turns and tool calls (script or not) are hidden.
...
test("test_session_end_matches_every_mode", () => {
    // Scenario: session-end terminators anchor each session's extent, so no mode may hide them.  Steps: the session-end fixture matches all six modes.
    for (const mode of Object.values(TIMELINE_FILTER_MODES)) {
        assert.equal(checkNodeMatchesFilterMode(sessionEnd, mode), true);
    }
});

// Task 127: keyword search predicate tests.
...
test("test_checkNodeMatchesSearchTerm_empty_term_matches_every_node", () => {
    // Scenario: a blank search box hides nothing — blank and whitespace-only terms match all.  Steps: "" and "   " match a user turn and a commit.
...
test("test_checkNodeMatchesSearchTerm_matches_turn_text_case_insensitively", () => {
    // Scenario: the term matches a turn's message text regardless of case.  Steps: the "hi" user turn matches "HI"; it does not match an unrelated term.
...
test("test_checkNodeMatchesSearchTerm_matches_tool_call_summary_and_name", () => {
    // Scenario: a tool-call row is searchable by its tool name and its one-line summary.  Steps: the git-mv Bash row matches "bash" and "git mv".
...
test("test_checkNodeMatchesSearchTerm_matches_commit_detail", () => {
    // Scenario: a commit row is searchable by its message detail.  Steps: the "wip" commit matches "wip"; the detail-less commit does not.
...
test("test_checkNodeMatchesSearchTerm_matches_file_chip_paths", () => {
    // Scenario: the driving use case — load s87, search "inventory", find the entry whose chip touched inventory.py (final, entry-time, and renamed-from names all count).  Steps: the rename-chip turn matches; the chipless agent turn does not.
...
test("test_checkNodeMatchesSearchTerm_hides_session_end_rows", () => {
    // Scenario: unlike the mode predicate, search has NO session-end exemption — a terminator carries no text, so a real term hides it ("non-matching entries hidden").  Steps: the session-end fixture fails a real term.
...
test("test_checkNodePassesFilters_requires_both_mode_and_term", () => {
    // Scenario: a row stays visible only when it passes the active mode button AND the term.  Steps: the rename-chip turn passes (files, "inventory"); fails on the wrong mode; fails on a non-matching term.
...
test("test_computeMatchingNodeIndexes_lists_matching_rows_in_timeline_order", () => {
    // Scenario: the search's jump list — the node indexes surviving the combined mode+search predicate, in timeline order (entry #1 of the results is the first index; N is the list's length).  Steps: among [rename-chip turn, plain agent turn, session end], "inventory" under All matches only the chip turn (index 0); a blank term matches all three.
...
test("test_merged_baseline_commit_row_matches_git_and_files_modes", () => {
    // Scenario (task 121): the merged baseline row is still a commit node AND carries file chips — Git mode matches it natively, Files mode matches through its chips.  Steps: the merged row matches git; the merged row matches files.
...
test("test_merged_baseline_commit_row_leaves_conversation_mode", () => {
    // Scenario (task 121, accepted trade): once merged, the baseline is no longer a turn — Conversation mode hides it. Pinned so the trade stays deliberate.  Steps: the merged row does not match conversation.
