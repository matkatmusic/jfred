// Tasks 247/248/249: a pair widget must span from its EARLIEST node instant to its LATEST, whichever KIND each of those happens to be. Split from tests/layer1-page.test.ts, which is at the 250-line cap; that file covers what a widget renders, this one covers how far it reaches.
//
// The bug these pin: `startPx` was `pair.commits[0].axisPx` unconditionally, so a file whose on-disk mtime PREDATES its first commit produced a negative `onDisk.axisPx - startPx`. The disk node is absolutely positioned in the lane with `translate(-50%, -50%)`, so a negative offset drew it above the lane's top — over the widget's own filename/sub header, or clear outside the bubble.

import { test } from "node:test";
import assert from "node:assert/strict";
import { flushAsyncWork, setupLayer1Dom, stubStreamRoute } from "./webapp-dom-test-helpers.ts";

// The pair's on-disk state, placed BEFORE either commit — the inverted case. A real 40-character hash, matching tests/layer1-page.test.ts: a short fake would pass a broken truncation unchanged.
...
// One pair whose disk mtime is older than its whole ladder, and nothing else — the two orphan buckets stay empty so every assertion below is about the one widget.
function buildDiskBeforeCommitsView(): object {
    return {
        pairs: [{ path: "src/stale.ts", commits: DISK_FIRST_COMMITS, onDisk: DISK_FIRST_ON_DISK }],
        gitOrphans: [],
        diskOrphans: [],
        // Each instant is drawn by exactly one of this pair's nodes, so every ruler entry counts one event (task 275 — the gutter prints it, nothing here reads it back).
...
// Boot a fresh page against a stubbed NDJSON stream whose terminal line is `view`, exactly as tests/layer1-page.test.ts does — bootLayer1Page is called explicitly because node's module cache runs the module's own boot line only on the first import.
...
test("test_a_widget_pins_to_its_earliest_node_when_the_disk_state_predates_its_commits", async () => {
    // Scenario (tasks 247-249): the widget's anchor is the earliest of ALL its nodes, not its first commit, so the bubble's top edge still lands on a real ruler tick and no node sits above it.  Steps: load the inverted view, whose on-disk instant is older than both commits.
...
test("test_no_node_in_a_widget_is_placed_at_a_negative_offset", async () => {
    // Scenario (tasks 247-249): a negative offset is what drew the disk node above the lane and over the header, so NO node may carry one — this is the assertion the old anchor failed.  Steps: load the inverted view.
    await loadPageWithView(buildDiskBeforeCommitsView());
...
test("test_a_widgets_lane_spans_to_its_latest_node_when_that_node_is_a_commit", async () => {
    // Scenario (tasks 247-249): the span was hardcoded to the on-disk node, so when the LAST node is a commit instead, the lane was too short to contain its own ladder.  Steps: load the inverted view, whose latest instant is its second commit.
    await loadPageWithView(buildDiskBeforeCommitsView());
