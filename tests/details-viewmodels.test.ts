// Pure view-model tests for the Details pane (webapp/views/details.ts, item 66): header text per node kind, 1-based revision cards, and the stored-diff-mode → toggle-label mapping.  Fixtures are wire-shaped literals (what the browser sees after fetch + JSON.parse); kind assertions go through the vocabulary enum members, never bare literals.  item 84's Revision View focus + range helpers are tested in details-revision-view.test.ts (this file is at the 250-line cap).
...
test("test_computeDetailsHeaderText_formats_message_nodes", () => {
    ...
    // Step 2: the header names the 1-based step, the total, the humanized kind, and the timestamp in the timeline's own toLocaleString format.
...
test("test_computeDetailsHeaderText_omits_missing_hash", () => {
    ...
    // Step 2: the hash segment is dropped entirely — a placeholder dash reads broken (user report, s58), so the header goes straight to the message.
    const expectedTimestamp = new Date("2026-07-01T11:00:00Z").toLocaleString();
    assert.equal(
        computeDetailsHeaderText(node, { index: 5, total: 9 }),
...
test("test_buildRevisionCards_numbers_revisions_and_carries_kinds", () => {
    ...
    // Step 2: cards are 1-based, and each carries its revision's kind, timestamp, and changeId (both revisions recovered, so no unrecoverable reason).
...
test("test_buildRevisionCards_flags_unrecoverable_revision", () => {
    // Step 1: a history whose second revision the engine could not replay (task 119) — its wire revision carries the unrecoverable placeholder marker.
    const history = {
        target: "src/orders.py",
        revisions: [
            { kind: EventKind.write as string, changeId: "toolu_A1", timestamp: "2026-07-01T10:00:00Z" },
...
test("test_resolveInitialFullContentsChoice_reads_the_stored_flag", () => {
    ...
    // Step 2: absent / "0" / anything else means off — full contents is opt-in, so an unset key reads as off (default is the ±3-line hunk view).
