...
test("test_path_wraps_value_compares_and_serializes", () => {
    // Scenario: a Path is a domain object around a filesystem path string, never a bare primitive — it exposes its value, compares by value, and serializes back to the raw string so the wire form round-trips.  Steps: build a Path and read its value.
...
test("test_uuid_wraps_any_id_value_compares_and_serializes", () => {
    // Scenario: a Uuid is a domain object around any identifier string — not only RFC-4122 values, but also prefixed ids (toolu_…, cse_…) — so no id is ever a bare primitive.  Steps: build Uuids from both an RFC-4122 value and a prefixed tool-use id.
