// The task-141 nav counter: 0-based index plus the full index range, never readable as "index over count". webapp/inspector.ts imports DOM-touching modules, so the happy-dom globals are installed before the dynamic import (the app-header.test.ts pattern).
...
test("test_formatInspectorLineCounter_names_the_zero_based_index_range", async () => {
    // Scenario (task 141): "line 0 / 98" was ambiguous (count or max index?); the label now shows the 0-based index AND the inclusive index range of a 99-line transcript.
    setupWebappDom();
...
test("test_inspector_nav_disabled_states_at_ends_and_middle", async () => {
    // Scenario (task 142): the record stepper's Prev disables on line 0 and Next disables on the last line, instead of both staying enabled and clamping to a silent no-op.
    setupWebappDom();
