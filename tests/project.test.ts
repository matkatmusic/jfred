// DOM test for webapp/views/project.ts (task 185): the File Nav drawer's JSONL list lives in
// its own scroll container capped at half the column, so a project with many JSONLs leaves
// the files-touched tree visible below it. (The DOM-free view-model tests for this module
// live in tests/viewer-project-views.test.ts.)
...
test("test_drawer_caps_jsonl_list_and_keeps_file_tree_outside_it", async () => {
    ...
    // Every JSONL link renders inside the capped .drawer-jsonl-list container — none as a
    // direct drawer child, or the CSS height cap would not bound them.
    ...
    // The files-touched tree renders OUTSIDE the capped container, so it stays visible no
    // matter how many JSONLs the project has.
...
test("test_stylesheet_caps_jsonl_list_at_half_the_column_and_scrolls_it", () => {
    // happy-dom computes no layout, so the 50% cap is asserted against the stylesheet rule
    // the container class carries.
