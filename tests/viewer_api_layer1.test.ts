...
// View — on-disk file state paired against a git tree. This file covers the PATH half (which paths
// land in `pairs` / `gitOrphans` / `diskOrphans`) plus the bad-input rejections. The axisPx
// PLACEMENT half is tests/viewer_api_layer1_placement.test.ts (task 240), split out when the
// task-240 cases pushed this file past the 250-line cap — precedent: tests/layered-app-widgets.ts
// carved out of tests/layered-app.test.ts for the same reason.
//
// The fixture (repo, folder, pinned instants) is shared with that file via
// tests/layer1-view-test-helpers.ts, but the viewer process is NOT: this file spawns and kills its
...
// (viewer_api_ladder.test.ts) and 19400 (viewer_api_layer1_placement.test.ts) are taken — parallel
// test files must never collide.
...
test("test_layer1_view_endpoint_pairs_disk_files_against_the_repo_tree", async () => {
    ...
    // `diskOrphans` holds exactly the on-disk paths with no repo entry, INSTANT-ascending rather
    // than in the disk walk's path order (which would read disk-only.txt first).
