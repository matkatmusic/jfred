...
test("test_readNonEmptyLines_returns_only_non_blank_lines", () => {
    // Scenario: readNonEmptyLines splits a file into lines and drops blank and whitespace-only lines.  Steps: write a file with blank and whitespace-only lines between content lines.
...
test("test_loadRecords_parses_every_non_blank_line_into_typed_records", () => {
    // Scenario: loadRecords reads a JSONL file and parses each non-empty line into a typed record (blank lines skipped).  Steps: write a two-record JSONL file with a trailing blank line.
