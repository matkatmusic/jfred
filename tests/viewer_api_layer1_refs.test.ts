// Server test for task 286: GET /api/layer1-refs?repo=&ref= feeds Layer 1's branch and commit dropdowns. The handler is called DIRECTLY with a captured response rather than over a spawned viewer — the route does no streaming, so a live server would only add a port to collide on.
//
// The repo fixture follows tests/layer1-view-test-helpers.ts's `makeFixtureRepo` (pinned committer dates, fixed identity) but is copied rather than imported: that one has exactly one branch, and this route's whole subject is the branch list.
...
// `git init -b` pins the initial branch name, so the head assertion does not depend on whatever init.defaultBranch the machine running the tests happens to carry.
const TRUNK_BRANCH = "trunk";
const SIDE_BRANCH = "sidecar";

// Both dates are stamped with the same instant, unlike layer1-view-test-helpers.ts's runGit which pins the author date far away: `--format=%ad` (the format viewer_api_repo.ts already parses) prints the AUTHOR date, so it is that one the commit dropdown shows.
...
// Two commits on `trunk`, plus a second branch that is NOT checked out — so "both branches present" and "head is the checked-out one" are different assertions rather than the same one twice.
...
test("test_layer1_refs_endpoint_rejects_a_folder_that_is_not_a_repository", () => {
    // Scenario: this throw IS the repo confirmation the dropdowns gate on — a folder that is not a repo must fail here, naming itself, so viewer_server.ts's outer catch turns it into a 400.
