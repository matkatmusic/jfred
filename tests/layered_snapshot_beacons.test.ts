// Task 201 (spec S4): collectSnapshotBeaconNodes — file-history snapshots become verified
// beacon nodes, each resolved through its OWNING session's sidecar (Q12): the same `abc123@vN`
// blob name in two sessions must yield each session's own bytes, never a global lookup's.
// Records are fabricated in memory (precedent: tests/reconstruction_sidecar.test.ts); the
// reader is a fake keyed by sessionId, so no disk file-history is ever touched.
...
// A synthetic envelope record carrying the transcript's cwd and its session's id — the
// snapshot records that follow it belong to this session.
...
// A file-history snapshot backing up `path` into blob `backupFileName` at `backupTime`
// (null backupFileName = version 1, which holds no blob).
...
test("test_collectSnapshotBeaconNodes_resolves_each_snapshot_through_its_owning_session", () => {
    // Scenario: sessions A and B both name their f.txt blob "abc123@v2"; each beacon must carry
    // ITS session's bytes — a global name lookup would hand one session's bytes to both.
    const records = [
...
test("test_collectSnapshotBeaconNodes_skips_null_backup_blobs", () => {
    // Scenario: a version-1 snapshot (null backupFileName) holds no blob and contributes no
    // beacon; a path with ONLY null backups gets no map entry at all.
    const records = [
        buildSessionCwdRecord("/work", SESSION_A),
