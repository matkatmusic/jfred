// Tests for the consent dialog's pure helpers (webapp/app.ts — DOM-free functions): script grouping, preview overflow, inline-interpreter splitting, header selection navigation, and the script source token.
...
test("test_groupConsentScriptsIntoBlocks_treats_missing_flag_as_modifying", () => {
    // Scenario: a script with no readOnly field (older server) renders as a full modifying row — the dialog degrades to today's behavior, never hides anything untagged.
...
test("test_checkConsentScriptOverflowsPreview_returns_false_for_short_script", () => {
    // Scenario: a script that fits inside the 200px preview needs no Expand button.  Steps: build a script whose line count is exactly the preview capacity (12 lines).
...
test("test_checkConsentScriptOverflowsPreview_returns_true_for_script_longer_than_preview", () => {
    // Scenario: a script one line taller than the preview capacity gets an Expand button.  Steps: build a script whose line count is one over the preview capacity (13 lines).
...
test("test_splitInlineInterpreterCode_splits_python_dash_c_wrapper_from_inline_body", () => {
    // Scenario: a recorded run stored as a full shell line (`python3 -c "…" 2>&1`) is split into wrapper + inline Python so the body can be highlighted in its real language.  Steps: build a shell line whose quoted body is real Python (single quotes inside are fine).
    const body = "\nimport json\npath = '/tmp/x.jsonl'\nprint(f'{path}')\n";
...
test("test_splitInlineInterpreterCode_returns_undefined_for_plain_python_script", () => {
    // Scenario: a normal recorded run is pure Python source, not a shell wrapper — the splitter must decline so the whole preview keeps highlighting as Python.
...
test("test_findDefaultConsentSelectionIndex_picks_first_modifying_script", () => {
    // Scenario: the consent header's default selection is the first modifying script.  Steps: a script list holds two read-only scripts followed by a modifying one.
...
test("test_findDefaultConsentSelectionIndex_treats_missing_flag_as_modifying", () => {
    // Scenario: a script without a readOnly flag counts as modifying (same rule as groupConsentScriptsIntoBlocks).  Steps: a script list holds one read-only script followed by one with no flag at all.
...
test("test_findDefaultConsentSelectionIndex_returns_undefined_when_all_read_only", () => {
    // Scenario: with no modifying script there is no default selection — every row starts hidden inside a closed read-only <details> block, so nothing is selectable on load.  Steps: a script list holds only read-only scripts.
...
test("test_clampConsentSelectionStep_enters_list_from_no_selection", () => {
    // Scenario: with no current selection (index -1, e.g. every row was hidden until a read-only block opened), stepping forward lands on the first visible script.
...
test("test_formatConsentSourceToken_formats_basename_and_line", () => {
    // Scenario: a script extracted from a known JSONL file + line renders as the same " [file.jsonl:123]" token the server's formatRecordSourceToken emits for console labels.  Steps: a source records the full transcript path and its 1-based line number.
...
test("test_formatConsentSourceToken_returns_empty_string_without_source", () => {
    // Scenario: a script whose record source was never captured contributes nothing to the muted header line — no empty brackets, no stray space.
    assert.equal(formatConsentSourceToken(undefined), "");
});

test("test_formatConsentSourceToken_keeps_bare_filename_unchanged", () => {
    // Scenario: a path with no directory separators is already a basename — extraction must pass it through untouched.
