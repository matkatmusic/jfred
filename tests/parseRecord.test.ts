...
test("test_parseRecord_hydrates_envelope_domain_fields", () => {
    // Scenario: a conversational record's envelope fields are hydrated into
    // domain objects — ids into Uuid, cwd into Path, timestamp into Date — so no
...
test("test_parseRecord_throws_on_unknown_record_type", () => {
    // Scenario: a record with a `type` outside the s1 vocabulary is rejected
    // loudly, so a future fog-of-war violation cannot pass silently.
    // Steps:
    // build a line whose type is not in the known set.
