// Tests for src/viewer_api_projects.ts blob-snapshot reads (trust boundary). Split out of viewer-api-projects.test.ts (split, never condense) to keep that file within the 250-line cap.
...
// A live (owning session, blob name) pair derived from the s43 capture: scan each transcript's file-history-snapshot records for trackedFileBackups entries and return the first whose blob file exists under the real ~/.claude/file-history root (reader-dependent, like the sidecar tests — there is no root override in this repo). undefined when none is on disk.  One transcript line of findExistingS43Backup's scan: parse a file-history-snapshot line's trackedFileBackups and return the first backup whose blob file is on disk, else undefined.
...
// One transcript of findExistingS43Backup's scan: check every line of the JSONL for an on-disk tracked backup, else undefined.
...
test("test_readBlobSnapshot_rejects_a_blob_name_with_path_separators", () => {
    // Scenario: both arguments reach a filesystem join, so anything that is not a bare `<16 hex>@vN` blob name (or a bare session id) is refused — traversal is impossible.
...
test("test_readBlobSnapshot_reports_a_nonexistent_blob_as_missing", () => {
    // Scenario: a well-formed name that is simply not on disk is not an error — the client renders it as "(missing from disk)", so the read reports { exists: false }.
...
test("test_readBlobSnapshot_reads_an_existing_blob_from_the_owning_session_dir", (t) => {
    // Scenario: a blob named by an s43 file-history snapshot reads back verbatim from its OWNING session's dir under ~/.claude/file-history (owner-dir only — no cross-session fallback, per the multi-session @vN collision fix).  Steps: derive a live (session, blob) pair from the s43 transcripts, then read it.
    const pair = findExistingS43Backup();
    if (pair === undefined) {
        // Machine-bound input (task 165): the live ~/.claude/file-history blobs exist only on the capture machine — absent (e.g. CI), skip rather than fail.
