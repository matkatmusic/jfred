// Task 238 (spec S18): the MILESTONE ACCEPTANCE TEST. Every other Layer 1 test exercises one half
// — the endpoint against a fixture, or the page against a canned view. This one runs the whole
// path: a real folder and a real UNRELATED repo, through the real GET /api/layer1-view on a
// spawned viewer, into the real page render in happy-dom. The page's relative fetch is forwarded
// to the live origin (forwardFetchToOrigin); nothing about the view is canned.
//
// Pixel offsets are deliberately NOT asserted here — tests/viewer_api_layer1_placement.test.ts
// owns placement. This file asserts the user's acceptance criterion: two buckets, zero pairs, and
// every path in the correct bucket.
...
// Own port range: 18900+ and 19400+ are claimed by the two viewer_api_layer1 files, each reserving
// 500 through `process.pid % 500`.
...
//
// bootLayer1Page() must run for EVERY document, not once: it is what seeds the header boxes from
// the page URL, and loadLayer1View reads those BOXES rather than the URL — an unseeded document
// makes it bail at its own missing-dir/repo guard and render nothing. node's module cache runs the
// module's own top-level boot only on the first import, so every later test needs this call.
//
// loadLayer1View is then awaited rather than flushAsyncWork()'d: that helper is three zero-delay
...
// A bucket's identity is its TITLE, never its position or its size — the two buckets are mirror
...
test("test_unrelated_roots_render_zero_pair_widgets", async () => {
    // Scenario (task 238, spec S18): when the folder and the repo share no relative path, there is
    // nothing to pair, so the stage draws no pair widget at all and says so.
    // Steps:
...
test("test_every_repo_path_and_every_disk_path_lands_in_its_own_bucket", async () => {
    // Scenario (spec S18 "Output contract"): every repo file belongs under "No on-disk match" and
    // every disk file under "No repository match". The two sets are mirror images, so this asserts
    // WHICH PATH is in WHICH bucket — a swap would leave both counts identical.
    // Steps:
    // render the page against the live endpoint with two unrelated roots.
    await renderPageAgainstLiveEndpoint();
