// Task 257.5 + task 294: clicking a node opens the Detail View drawer on that node's bytes, and those bytes are rendered by layer1-file-view.ts — numbered column plus highlighted code.
//
// The stage is built by hand rather than through the page, the precedent tests/layer1-tick-files.test.ts sets: the contract the drawer reads is the DOM layer1-page.ts renders (`.filebox` > `.fname[data-path]`, a `.lane` holding `.node` + `.nlabel`), and building it directly is what makes a one-node bubble three lines instead of a whole fetched view.
...
// setupLayer1Dom publishes happy-dom's document but not its animation clock, and the drawer defers its re-centre onto one — so the callback is run inline here. Nothing it does (scrollIntoView, the minimap redraw) is measurable without layout, which happy-dom has none of; running it at all is what proves the drawer does not throw on the way to the fetch.
...
// One pair bubble carrying a single on-disk node — the kind that reads the working tree, so the stubbed route needs no commit hash.
...
// The drawer's fetch is awaited inside the click handler, so the assertions have to run after the microtask queue drains — one turn of the event loop is enough for a stubbed, synchronous route.
...
test("clicking an on-disk node opens the drawer on that file's bytes", async () => {
    ...
    // Task 294's shape: two lines counted, the file's text in the code column, the trailing newline NOT numbered as a third row.
...
// A commit bubble: the dot carries the FULL hash on its title, which is what the drawer reads to build `git show <hash>:<path>`. buildPairWidget puts it on the dot and its label alike.
...
test("a commit node asks git for the blob at its own hash", async () => {
    // User, 2026-07-27: the drawer read the hash off the LABEL-only title, saw "", and asked for `git show :<path>` — which the route reads as the on-disk form and refuses for a missing `dir`. The reported error named the wrong parameter entirely, so this pins the URL.
    setupLayer1Dom();
    stubAnimationFrame();
    let asked = "";
