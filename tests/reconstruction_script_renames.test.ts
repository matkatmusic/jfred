...
// A printed `old -> new` line for a previously-written file becomes one rename, resolved against the run's cwd and stamped at the RESULT record's instant — while the echoed f-string code line `{name}.py -> …` (braces are not path chars) in the SAME result yields nothing.
test("test_extract_script_rename_pulls_printed_move_and_ignores_code_echo", () => {
    ...
    // Stamped at the RESULT record — the execution provably finished by then. The tool_use instant is only when the run was REQUESTED: s87's consent-delayed MCP move sat pending 6 minutes.
    assert.equal(renames[0]!.timestamp.getTime(), new Date("2026-01-01T00:00:03Z").getTime());
});

// Consent-delayed execution (s87 steps 110–118): an MCP run REQUESTED at 00:24 whose result only lands at 00:31 executed somewhere in between — Bash runs at 00:30 prove the file had not moved yet. The rename must stamp at the result instant so those interleaved events order BEFORE it.
...
// The `Renamed: <old> -> <new>` prefix form (s84) is matched by the arrow anywhere in the line, and the Bash `.stdout` result shape is read the same as the MCP array shape.
...
// Code-literal channel (s87 step 89): a COMPLETED run whose code contains a two-string-literal `shutil.move("a.py", "b.py")` yields a rename even when the run prints NO `old -> new` line.  The variable form `shutil.move(src, dst)` in the same code yields nothing (arguments unknown).
...
// A code-literal move in a run with NO tool_result (never confirmed to complete) yields nothing — the never-fabricate rule: only a KNOWN completed run's code counts as rename evidence.
...
// Chain-aware phantom guard (s87 step 106): run 2 renames a file BORN as run 1's destination.  Records arrive in readdir order (run 2's records BEFORE run 1's), so a record-order guard sees run 2's source as never-written and drops it. Candidates must be ordered by EXECUTOR timestamp, each accepted source ∈ written basenames ∪ destinations of already-accepted renames.
