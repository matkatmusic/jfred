// Task 259: a run of nodes in ONE bubble that share an instant must be wrapped in a single grouping
// marker. Task 251 stopped a commit and an on-disk mtime at the same instant from overprinting by
// giving each its own 22 px row — but once they are on separate rows nothing says they were
// simultaneous, and this marker is what restores that.
//
// buildTieGroupMarkers is called DIRECTLY rather than through a page boot: the behaviour under test
// is pure element construction, so a stubbed NDJSON stream would only widen the failure surface.
...
// task 251's axis charges that instant two stacked rows — one RULER_NODE_ROW_PIXELS (22) apart.
...
// Every widget's nodes are drawn relative to its own anchor, which is its earliest node — here the
// first tied node.
...
test("test_two_nodes_sharing_one_instant_are_wrapped_in_one_group_marker", async () => {
    // Scenario (task 259): a commit and an on-disk mtime at one instant occupy two stacked rows;
    // exactly one rectangle must be drawn around them.
    // Steps:
    // build the markers for a two-node ladder whose instants are equal.
...
test("test_nodes_at_distinct_instants_get_no_group_marker", async () => {
    // Scenario (task 259): the overwhelming majority of pairs have no tie at all, and those bubbles
    // must render exactly as they did before this task.
    // Steps:
    // build the markers for a ladder whose two nodes are a day apart.
    const { buildTieGroupMarkers } = await import("../webapp/layer1-tie-groups.ts");
...
test("test_a_run_of_three_tied_nodes_yields_one_marker_spanning_all_three", async () => {
    // Scenario (task 259): a run is grouped as a WHOLE — grouping pairwise instead would draw two
    // overlapping rectangles over the same three rows.
    // Steps:
    // build the markers for a ladder of three nodes that all share one instant.
    const { buildTieGroupMarkers } = await import("../webapp/layer1-tie-groups.ts");
