...
//
// What is asserted is the `shown` CLASS, exactly as tests/layer1-leader-hover.test.ts asserts `aimed`: layer1-styles.css is what turns the class into an opacity, happy-dom implements no cascade, and a measured opacity would be measuring happy-dom rather than the page.
//
// No getBoundingClientRect stub is needed here — unlike the `aimed` hover, showing a line has no visibility gate, so this behaviour is fully observable without layout.
...
// REAL 40-character hashes: the page shortens the visible label, and a 7-character fake would let a broken truncation through unnoticed.
...
// Drive the hover the way the delegated listener sees it, rather than calling the resolver: the listener is wired to the PANE, so a bubbling event is what proves the wiring as well as the lookup.
...
test("test_no_leader_line_is_drawn_until_something_asks_for_one", async () => {
    // Scenario (the user's rule): "for all other scroll events in the timeline, no dashed lines should be drawn". A freshly rendered stage has had no hover and no selection, so the rest state must be empty — this is the assertion the whole change exists for.  Steps: render two pairs over a three-entry ruler.
...
test("test_hovering_a_ruler_timestamp_draws_that_row_s_line", async () => {
    // Scenario: the first of the three sources the user named — "the user mouses over a ruler timestamp". A tick carries the ABSOLUTE offset, so this is the direct case.  Steps: render, then mouse onto the gutter row for the shared instant.
    await loadPageWithView(buildSharedInstantView());
...
test("test_hovering_a_bubble_draws_the_line_for_the_instant_it_begins_at", async () => {
    // Scenario: "mouses over ... a bubble". A bubble's own --axis-px is its FIRST node's offset, so hovering its header must draw the line for where it begins — EARLY_PX for the spanning pair, even though that same bubble also holds a node on the shared instant.  Steps: render, then mouse onto the spanning bubble's name.
    await loadPageWithView(buildSharedInstantView());
...
test("test_hovering_an_interior_node_draws_that_node_s_line_not_its_bubble_s", async () => {
    // Scenario: "mouses over ... a node" — and the case that catches the arithmetic. A node's --axis-px is WIDGET-RELATIVE, so the spanning bubble's second node reads 30 while the line it belongs to is at 40; only adding the bubble's own base back on resolves it.  Steps: render, then mouse onto the second node of the bubble that BEGINS 30px earlier.
    await loadPageWithView(buildSharedInstantView());
...
test("test_a_landed_selection_keeps_its_line_up_after_the_pointer_moves_away", async () => {
    // Scenario: "or selects a bubble [or] a node". A landing — the find box, a File Nav leaf or a ruler-tick click, all of which funnel through highlightLandedElement — is a SELECTION, so its line has to outlive the pointer that is no longer anywhere near it.  Steps: render, then land on the bubble that begins at the shared instant, as the find box would.
    await loadPageWithView(buildSharedInstantView());
