// Task 233 (spec S18): listPairCommitHistory — one node per commit that TOUCHED a pair's path, none for the commits that did not. Fixture is a real temp repo of four commits where the target is touched by exactly two of them, with committer and author times deliberately set to DIFFERENT days so a run that read author time would be caught by the instant assertions.
...
// Run a git command in `repoDir` with a fixed identity; the two dates are pinned apart so the module's time source is observable — committer 2026-07-0X, author always 2020-01-01.
...
// Four commits; nested/target.txt is created by the first and edited by the third. Commits two and four touch other.txt only, so they must contribute nothing to the target's history.
...
test("test_only_the_commits_that_touched_the_file_become_nodes", () => {
    // Scenario: of four commits, two touched nested/target.txt — so the history has exactly two entries, in commit order (oldest first), and the untouching commits leave no empty node.
...
test("test_history_instants_come_from_committer_time_never_author_time", () => {
    // Scenario: every commit in the fixture was authored 2020-01-01 and committed on its own 2026-07 day. The nodes must carry the 2026 committer instants — an author-time read would collapse both onto 2020-01-01 (S18 / hpp Q7: one machine, one clock).
...
test("test_a_path_no_commit_ever_touched_has_an_empty_history", () => {
    // Scenario: a repo-tracked-looking path that no commit touched contributes no nodes at all rather than a placeholder — the same posture the untouching commits get above.
    const { repoDir } = makeFourCommitRepo();
...
test("test_a_renamed_file_shows_only_its_post_rename_history", () => {
    // Scenario: S18 forbids `--follow` because rename tracking belongs to S6. After renaming nested/target.txt to nested/renamed.txt, the new path shows ONLY the rename commit — the two pre-rename commits stay with the old path. This is the documented shorter history.
    const { repoDir } = makeFourCommitRepo();
