// The shared harness for the Layer 1 find box's tests (task 261, extended by tasks 271/273/280).
// Split out of tests/layer1-find-file.test.ts when tasks 271 and 273 pushed that file past the
// repo's 250-line cap: what moved here is only the SETUP — the DOM boot, the scroll spy, the bubble
// fixture and the two readouts — so every test and every one of its scenario comments stayed where
// it was.
//
// happy-dom implements NO layout and NO scrolling, so nothing here reads a scroll position — that
// would be measuring happy-dom rather than the page. Instead scrollIntoView is SPIED on: the
...
// tell the six same-basename bubbles apart. Exported by reference and emptied in place by
// openFoundPage — never reassigned, so an importing test keeps seeing the same array.
...
// (task 245) and the full path as that element's `data-path` (task 280 moved it off `title`). The
// box must match what the user can SEE, which is only the basename — so a test fixture that put the
// full path in the text would prove nothing.
...
// A booted page whose stage holds `paths` as bubbles. A fresh DOM is NOT enough on its own: the
// cycle position and the term it belongs to are module scope, so they outlive the document — two
// tests searching the same term would otherwise have the second one resume where the first stopped.
// Emptying the box is the page's OWN reset (task 273), so the leak is closed by driving that rather
// than by exporting a hook that exists only for the tests.
...
// header's shared #crumb — which is what makes clearing it one empty string rather than a
// save-and-replay of whatever the stage had written into the crumb.
