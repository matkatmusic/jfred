// Task 290, second half (user, 2026-07-26): "when you mouse over a leader line and the node(s) or
// bubble(s) it is pointing to is/are in the visible part of the timeline, highlight those nodes and
// make the dashed leader line's color change to the same highlight color used for nodes and bubbles."
//
// What is asserted is the `aimed` CLASS. layer1-styles.css is what recolours the dashes and marks the
// targets from it, so the class IS this module's contract — happy-dom implements no cascade, and a
// measured colour would be measuring happy-dom (same reasoning as tests/layer1-ruler-click.test.ts).
//
// happy-dom also implements no LAYOUT, so getBoundingClientRect answers an all-zero rect for every
// element and the visibility gate would say false for everything — the feature is unobservable
// without a stub. Every test here patches that prototype method AFTER booting, exactly as
// tests/layer1-ruler-click.test.ts patches scrollIntoView, because setupLayer1Dom builds a FRESH
// window per test and an earlier window's prototype is not the one this page's elements use.
...
// SHARED_PX is the crux: the second bubble BEGINS there while the first merely holds an interior
// commit node on it, so a leader hovered there must light BOTH kinds of target. Every offset is >=
// the page's 13 px tick-label collision threshold from its neighbour, so no ruler entry is dropped.
const EARLY_PX = 10;
const SHARED_PX = 40;
const LATE_PX = 110;
...
// Drawn SECOND, and it BEGINS on the shared instant — so the hover's target set has to hold a
// `.filebox` as well as the node above it.
...
// Both pairs on one ruler. Every entry carries task 275's event count because the gutter refuses to
// draw a row without it; one leader line is drawn per ENTRY, which is what this file hovers.
...
// Only the four edges are read, so a plain box is enough — happy-dom never constructs a real DOMRect
// here because it has no layout to construct one from.
...
// One box every element shares, and one the PANE alone gets: disjoint vertically, which is the
// scrolled-away case — the targets are still in the document, just outside the visible scrollport.
...
// The dashed line drawn for one absolute offset. Fails loudly rather than returning undefined: a
// test aimed at a line the render never produced is testing nothing.
...
test("test_hovering_a_leader_line_highlights_every_visible_target_at_its_instant", async () => {
    // Scenario (task 290, the user's words): "I cannot visually tell what the dashed leader lines are
    // actually pointing at when they intersect with a bubble or node." A leader marks the DOTS it
    // points at — the nodes drawn at its instant, in EVERY bubble that touches it, whether that
    // bubble begins there or merely holds it. Never the bubble's box: a lit box does not say which
    // instant it means, which is the same complaint the ruler-tick click was fixed for on 2026-07-26.
    // Steps:
    // draw both pairs, then put every box on screen so the visibility gate can pass.
    ...
    // ...and BOTH dots at that instant are: beginning.ts's first node and the interior node spanning.ts
    // merely holds there. A node's own --axis-px is widget-relative, so each of these resolves to the
    // hovered instant only if its bubble's base offset is added back on.
...
test("test_leaving_a_leader_line_drops_the_highlight", async () => {
    // Scenario (task 290): this is a HOVER state over MANY elements, not tasks 283/267's single
    // persistent selection — so leaving must darken everything entering lit, or the page would
    // accumulate one lit set per line the pointer ever crossed.
    // Steps:
    // draw both pairs, put every box on screen, and hover the shared instant's line.
    await loadPageWithView(buildSharedInstantView());
    stubEveryRectOnScreen();
...
test("test_a_leader_whose_targets_are_scrolled_away_stays_unhighlighted", async () => {
    // Scenario (task 290, the user's condition): the recolour is promised only "when the node(s) or
    // bubble(s) it is pointing to is/are in the visible part of the timeline". A line whose targets
    // have scrolled out of the pane must stay grey rather than advertise a target the reader cannot
    // find on screen.
    // Steps:
    // draw both pairs, then move the PANE's box clear of every target's box.
    await loadPageWithView(buildSharedInstantView());
