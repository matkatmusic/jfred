// Step 3 of the S18 Layer 1 feedback fixes: buildLayer1View announces its position while it runs.
// The route blocks for ~10 s against the real jfred repo — one `git log` per tracked path — and the
// page showed the PREVIOUS view's crumb the whole time, which read as frozen.
//
// Called IN-PROCESS rather than through a spawned viewer: the sink is a plain function parameter,
// so nothing here needs HTTP, a scratch port, or the NDJSON framing (tests/viewer_api_layer1_stream
// .test.ts owns that). The fixture is the shared one every other Layer 1 server test uses.
...
test("test_buildLayer1View_reports_a_counted_event_per_pair_history_read", () => {
    // Scenario: the endpoint blocks for ~10 s on one `git log` per tracked path, so it must
    // announce its position so the page can draw a determinate bar.
    // Steps:
    // build the view over the shared fixture with a recording sink.
...
test("test_buildLayer1View_reports_a_counted_event_per_repository_only_placement", () => {
    // Scenario: placing the repo-only bucket costs one `git log` per row too, so it is counted on
    // its own rather than folded into the pair count — the two stages have different totals.
    // Steps:
    // build the view over the shared fixture, whose repo holds one path absent from disk.
...
test("test_buildLayer1View_announces_every_countless_stage_in_execution_order", () => {
    // Scenario: the walk and the tree read happen BEFORE any counted work, and the ruler resolve
    // after all of it, so a countless label keeps the bar moving through stages that have no N.
    // Steps:
    // build the view and keep only the stages that carry no count.
    const events = recordProgressWhileBuilding();
...
test("test_buildLayer1View_needs_no_sink_at_all", () => {
    // Scenario: the sink is optional, so every pre-existing caller (and the plain non-streaming
    // route) keeps compiling and running untouched.
    // Steps:
    // build the view with three arguments, exactly as the plain route does.
