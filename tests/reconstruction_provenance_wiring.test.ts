...
test("test_reconstructStepStates_with_provenance_enabled_records_instrumented_stage_entries", () => {
    // Behavior: s29 (a script-rename whose post-script beacons need backup completion) drives instrumented
    // beacon stages during the SAME per-step reconstruction the coverage tool runs, so provenance enabled
    // around reconstructStepStates captures at least one entry and every entry names one of the six
    // instrumented stages — proving the sink is wired into the step path checkScenario uses.
