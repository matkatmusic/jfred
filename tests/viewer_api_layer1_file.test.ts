// Task 257 step 7: GET /api/layer1-file serves ONE file at ONE moment for the Detail View drawer.  The two forms must not agree — a commit node shows the COMMITTED bytes while the on-disk node shows what is on disk NOW — so the fixture is one committed file whose working-tree copy has since changed, and each assertion names the bytes only its own form can produce.
//
...
//
// The temp-repo fixture is modelled on tests/viewer_api_layer1.test.ts's (via tests/layer1-view-test-helpers.ts) and deliberately COPIED rather than imported: those helpers build the pinned-instant Layer 1 ruler fixture that three files depend on, and this test needs a plain repo with a dirty working tree instead.
...
// Run a git command in `repoDir` with a fixed identity, so the fixture never depends on the machine's git config.
...
// One repo holding one committed file that has since been edited on disk — the exact state the drawer distinguishes between.
...
test("test_layer1_file_endpoint_reads_the_committed_bytes_for_a_commit_node", () => {
    // Scenario: a commit node is clicked, so the drawer must show the file AS COMMITTED.  Steps: the hash form answers 200 with the committed text...
...
test("test_layer1_file_endpoint_reads_the_working_tree_bytes_for_the_on_disk_node", () => {
    // Scenario: the on-disk node is clicked, so the drawer must show what is on disk NOW — the dir form takes no hash and reads straight through git.
...
test("test_layer1_file_endpoint_refuses_a_path_that_escapes_the_folder", () => {
    // Scenario: `path` arrives from a URL, so traversal is a trust boundary, not a typo.  a ../ path is refused before any read, naming the offending path.
    assert.throws(
        () => requestLayer1File({ dir: repoDir, path: "../../etc/passwd" }),
        /escapes the folder/,
    );
    // a bare ../ that lands next to the folder is refused too — the sibling case a prefix comparison without a separator would let through.
    assert.throws(
