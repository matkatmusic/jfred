// Task 252: the Layer 1 File Nav must list EVERY identified file — the paired files, the repo files
// with no on-disk presence, and the on-disk files the repo does not have — using the webapp's
// existing file tree rather than a second one written for this page.
//
// The nav builder is called DIRECTLY rather than through a page boot: the behaviour under test is
// the payload -> entries mapping and the tree it renders, so a stubbed NDJSON stream would only
...
// One of each kind the endpoint returns. All three share `src/`, which buildFileTree STRIPS as the
// common prefix, so one path is nested a level deeper — otherwise the tree collapses to three root
// leaves and the folder assertion below would be testing the fixture rather than the renderer.
const NAV_VIEW = {
...
test("test_every_identified_file_appears_in_the_file_nav", async () => {
    // Scenario (task 252): the pane lists all three kinds of identified file, including the ones
    // with no current on-disk presence.
    // Steps:
    // render the nav for a view holding one pair, one repo-only path and one disk-only path.
...
test("test_a_repo_file_with_no_on_disk_presence_renders_as_deleted", async () => {
    // Scenario (task 252): gitOrphans are in the repo tree and absent from disk, so they are the
    // struck-through rows. The two orphan sets are mirror images, so a swap would be invisible
...
test("test_a_pairs_commit_count_is_its_revision_count", async () => {
    // Scenario (task 252): Layer 1 has no revision ladder, so the count the shared leaf renders is
    // the pair's commit count.
    // Steps:
    // render the nav for a pair carrying two commits.
    const container = await renderNavIntoNewContainer();
...
test("test_the_nav_renders_folders_as_native_details", async () => {
    // Scenario (task 252): the pane must REUSE the existing tree, whose folders are native
    // <details open> elements — a flat list quietly written for this page would fail here.
    // Steps:
    // render the nav, one of whose paths sits a directory below the stripped common prefix.
    const container = await renderNavIntoNewContainer();
