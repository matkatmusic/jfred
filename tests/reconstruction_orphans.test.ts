...
// A PreToolUse-hook command rewrite (e.g. rtk) forks the tree mid-tool-call: the same tool_use "A"
// gets TWO tool_result children — the dead one (TR1, with its own last-prompt head) and the live
// one (TR2) the conversation continues from. The dead side is pure plumbing: no user prompt, no
// assistant text.
...
// A hook-rewrite micro-fork (dead side = tool_use/tool_result plumbing, no prompt, no assistant
// text) is not a /rewind — nothing dims (the rtk pattern).
