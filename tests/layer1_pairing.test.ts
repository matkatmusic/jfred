...
test("test_pairing_matches_on_the_full_relative_path_not_the_bare_name", () => {
    ...
    assert.deepEqual(pairing.repoOnlyPaths.map((path) => path.toString()), ["lib/app.ts"]);
    assert.deepEqual(pairing.diskOnlyPaths.map((file) => file.relativePath.toString()), ["src/app.ts"]);
});
...
test("test_an_unrelated_repo_yields_zero_pairs_and_two_full_buckets", () => {
    ...
    assert.deepEqual(pairing.repoOnlyPaths.map((path) => path.toString()), ["README.md", "src/main.rs"]);
    assert.deepEqual(pairing.diskOnlyPaths.map((file) => file.relativePath.toString()), ["photo.png", "docs/plan.md"]);
});
...
test("test_identical_trees_yield_pairs_and_no_buckets_at_all", () => {
    ...
    assert.deepEqual(pairing.repoOnlyPaths, []);
    assert.deepEqual(pairing.diskOnlyPaths, []);
});
