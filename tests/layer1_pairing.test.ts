// Task 232 (spec S18): pairDiskFilesAgainstRepoPaths — the exact-relative-path join of the `current file state` against the `starting repository state`, and the two orphan buckets that fall out of it. Pure data in, pure data out: no temp folder and no temp repo are needed, which is the point — the join must not consult the filesystem or git for anything.
//
// PATHS ONLY, deliberately (user-settled 2026-07-25): these cases assert what gets extracted, matched and orphaned — never axisPx, timestamps or ruler data. Vertical placement is task 240's separate test. They also pin the property NAMES `pairs` / `gitOrphans` / `diskOrphans`, because the two orphan sets are mirror images and an inversion would be invisible to the task-238 acceptance test, which only counts two buckets either way.
...
// One `current file state` entry. The mtime only has to be distinct per path here — pairing never reads it, it just has to survive the join so the on-disk node keeps its timestamp.
...
test("test_pairing_matches_on_the_full_relative_path_not_the_bare_name", () => {
    // Scenario: notes.txt sits at the root of BOTH trees, so it pairs; src/app.ts is on disk but the repo has lib/app.ts — same basename at a different path, which S18 says is NOT a pair.
...
test("test_pairing_keeps_each_paired_files_own_mtime", () => {
    // Scenario: a pair carries the disk entry itself, mtime included — the widget's on-disk node is drawn at that instant, so the join must not flatten a pair down to its path.
...
test("test_an_unrelated_repo_yields_zero_pairs_and_two_full_buckets", () => {
    // Scenario (the core of the task-238 acceptance test): the disk folder and the repo share no path at all. Every disk file lands in "No repository match", every repo path in "No on-disk match", and not one pair is invented.
...
test("test_identical_trees_yield_pairs_and_no_buckets_at_all", () => {
    // Scenario: every path matches, so both buckets come back empty — which is how the renderer learns to omit them (S18: buckets are omitted when empty).
