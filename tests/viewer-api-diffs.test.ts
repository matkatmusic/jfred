...
test("test_renderRevisionDiff_shows_consecutive_changes_for_a_file", () => {
    // Scenario: the revision-timeline diff for s19's scenario19.py contains the user's out-of-band tweak line as an addition.
...
test("test_renderDiffVsBase_diffs_first_revision_against_selected", () => {
    // Scenario: diffing the base (first revision) against the final revision of s19's scenario19.py shows the multiply function that only exists at the end.
    const { document, target } = buildS19DocumentAndTarget();
