// Task 264: the dashed leader line belongs to the RULER ENTRY, not to the bubble. Split from
// tests/layer1-page.test.ts, which is at the 250-line cap — same reason tests/layer1-page-spans.ts
// was split off before it.
//
// The bug these pin: the leader used to be `.filebox::before`, so it was drawn once per BUBBLE.
// Bubbles sharing a start instant each painted their own overlapping copy, and once the line had to
// be long enough to reach the sticky gutter from anywhere on the canvas (task 262 round two), 805
// copies of a 500,000 px dashed border made the page unusably slow (task 263). One line per ruler
// entry, spanning the canvas behind everything, is both the correctness fix the user asked for and
// the performance fix.
...
// drew two overlapping leaders at one offset. Real 40-character hashes, matching the sibling files —
// a short fake would pass a broken truncation unchanged.
...
test("test_two_bubbles_sharing_one_instant_share_a_single_leader_line", async () => {
    // Scenario (task 264, user's words): "there should be at most one dashed line for every entry in
    // the ruler, regardless of how many timeline bubbles that start at that timestamp".
    // Steps:
    // load two pairs that BOTH begin at the 0 px instant.
...
test("test_a_leader_is_drawn_even_where_the_tick_label_was_skipped", async () => {
    // Scenario (task 264): the overprint skip drops a LABEL, not an instant. Bubbles still sit on a
    // skipped instant, so dropping its leader too would leave them with no connector to the gutter.
    // Steps: the same crowded ruler, whose middle label does not survive.
    await loadPageWithView(buildSharedInstantView(CROWDED_RULER));
...
test("test_no_leader_is_drawn_inside_a_bubble", async () => {
    // Scenario (task 263): the performance fix is structural — nothing per-bubble may draw a leader
    // any more, or the 805-overlapping-copies cost comes straight back. The leaders live in their
    // own container OUTSIDE the stage, which is also what lets them span the sticky ruler gutter
    // (they are positioned against `.canvas`, whereas `.stage` begins to the RIGHT of the gutter).
    // Steps:
    await loadPageWithView(buildSharedInstantView(CROWDED_RULER));
