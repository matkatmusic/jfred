// Direct tests for the task-139 shell write-primitive pattern (the python patterns in this module are exercised end-to-end through tests/reconstruction_script_prestate.test.ts).
...
test("test_shellWritePrimitive_matches_write_verbs_and_file_redirects", () => {
    // Scenario (task 139): each shell write channel the gate must flag — a write-verb word, `sed -i`, and an output redirect whose target looks like a file path.
...
test("test_shellWritePrimitive_ln_word_requires_command_position", () => {
    // Scenario (task 192, optimizations.md Phase 2): the `ln` word must match only as a real command, never as the tail of an option cluster or an identifier.  Steps: `grep -ln pattern file` is a read-only search — the `ln` letters sit inside `-ln`.
...
test("test_shellWritePrimitive_ignores_read_only_probe_shapes", () => {
    // Scenario (task 139 guard): fd dups, /dev/null redirects, and plain probe commands carry no write channel — they must stay unmatched so the item-68 sandbox skip keeps firing.
