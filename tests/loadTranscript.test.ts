...
test("test_s1_jsonl_parses_into_known_typed_records", () => {
    // Scenario: loading the whole s1 transcript yields only typed records — no unknown record type and no unmodeled top-level key falls through.  Steps: load every record of the s1 JSONL through the gate.
...
test("test_s2_jsonl_parses_into_known_typed_records", () => {
    // Scenario: the whole s2-move-file transcript passes the field-level gate — its new vocabulary (Read/Edit tools, 3 new attachment kinds, the user isMeta key) is modeled, so nothing falls through as unknown/unmodeled.  Steps: load every record of the s2 JSONL through the gate.
...
test("test_s32_jsonl_parses_with_mcp_attribution_keys", () => {
    // Scenario: the s32 transcript runs its rename script through the context-mode MCP sandbox. The MCP-invoking assistant records carry two novel top-level keys (attributionMcpServer / attributionMcpTool). The field-gate must model them, not throw.  Steps: load every record of the s32 JSONL through the gate (throws UnmodeledFieldError at HEAD).
    ...
    // each carries the context-mode server; the tool name is run-specific (the re-run carries both ctx_execute and ctx_execute_file), so assert only that it is present.
...
test("test_s19_rerun_jsonl_parses_with_attribution_plugin_and_skill_keys", () => {
    // Scenario: the re-run s19 transcript was produced by a newer Claude Code that emits two novel top-level keys on assistant turns issued under an active skill (attributionPlugin / attributionSkill, same `attribution*` family as the MCP keys). The field-gate must model them.  Steps: load every record of the re-run s19 JSONL through the gate (throws UnmodeledFieldError at HEAD).
...
// The parseTranscriptLine field-gate tests (observed real-session fields, fork-context-ref, unmodeled-key rejection) live in tests/recordKeys.test.ts (250-line cap split).
