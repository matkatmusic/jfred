// The viewer's diff/patch rendering layer: per-file revision diffs, diff-vs-base, the range patch over step snapshots, and the trust-boundary query parsing for those routes. Pure functions over the existing engine — the HTTP wiring lives in viewer_server.ts.
...
// The revision-timeline view's text: consecutive-revision diffs for one file, with unified hunks + context so the client can render surrounding lines and line-number gutters.
...
// The directory every range-patch path is relativized against: the longest common directory prefix across every file the reconstruction ever tracked — in practice the session's cwd, since every tracked file lives under it. Stable for a given reconstruction regardless of the range.
...
// Trust-boundary parsing for GET /api/range-patch: both step params are required 1-based positive integers. Range validation against the document happens in renderRangePatch (it knows steps.length).
...
// The { path: content } map of every file present at a 1-based step, resolved ON DEMAND from the compact histories (skeleton steps carry no file map). One step's map is one repo snapshot — bounded, not multiplied. Loud throw (server -> 400) when the step is out of range.
...
// One git-apply-able unified diff covering every file whose content differs between the snapshot BEFORE `fromStep` and the snapshot AT `toStep` (1-based step indexes; the snapshot before step 1 is empty). A path present only in `after` is a creation; only in `before`, a deletion.
export function renderRangePatch(stepFileHistories: FileHistory[], steps: StepSnapshot[], fromStep: number, toStep: number): string {
    ...
    // The snapshot BEFORE step 1 is empty; otherwise resolve each endpoint's files on demand from the compact histories (no per-step file map exists on the document anymore).
