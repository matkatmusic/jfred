...
// turn; the LAST file-history-snapshot whose CONTENT signature differs from the previous one marks the
// last time the working tree actually changed. Content identity is the carried-forward `backupFileName`
// per path, NOT the `version` counter: a `code` restore with no post-edit emits trailing "refresh"
// snapshots that bump each file's version while leaving its content (and backupFileName) unchanged, so
// version would falsely report a change. The owner's `messageId` names the conversation record that
// produced the surviving files. See plans/s9/s9-reconstruction-plan.md (refines plans/s8/).
