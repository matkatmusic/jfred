...
// The verdict a single tool_use block earns: Write/Edit by name; Bash by whether its command is a file op
// (non-file-op bash -> ignore); any other tool (e.g. an MCP ctx_execute) -> undefined here. The
// `scriptExecution` branch (the ctx_execute rename idiom) lands with C5's `isScriptRenameRun`.
...
// Whether a user record's `toolUseResult` is a Read result carrying file content (a post-edit beacon the
// reseed stages read). The Read result nests content under `file.content` (tool-results.ts ReadResult).
function isReadResult(result: Record<string, unknown>): boolean {
    const file = result.file as { content?: unknown } | undefined;
    return typeof file?.content === "string";
}

// Whether a user record's `toolUseResult` is an Edit result (structuredPatch + before/after strings). The
// before/after strings distinguish it from a Write result, which also carries a structuredPatch.
...
// The verdict a record's `toolUseResult` earns: a Read result is a read-beacon, an Edit result is an
// edit-result; a Write/Bash result yields no verdict (the Write tool_use itself is the `write` line).
...
// Classify one raw JSONL line into a single Verdict. Anything not otherwise classified — prose, thinking,
// hooks, a Write/Bash result, a non-file-op bash — is `ignore`. A throw anywhere (bad JSON, unmodeled
// block) is treated as `ignore`: it cannot be evidence.
...
// A kept line: its 1-based file line number, its verdict, and the raw line (retained for the on-demand
// detail render). An ignored line carries no verdict (it is `ignore` by definition).
...
// Read a transcript and split it into kept / ignored lines. `lineNumber` is the 1-based file line number
// (so it matches how the transcript is referenced everywhere else). Blank lines are dropped entirely.
