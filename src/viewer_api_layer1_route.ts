...
function streamLayer1View(response: ServerResponse, projectFolder: Path, repoDir: Path, ref: string): void {
    ...
    try {
        response.end(JSON.stringify(buildLayer1View(projectFolder, repoDir, ref, writeNdjsonLine)) + "\n");
    } catch (error) {
...
// GET /api/layer1-view?dir=&repo=&ref=&progress= — the S18 Layer 1 View. Without `progress=1` it is
...
export function handleLayer1ViewRequest(response: ServerResponse, query: URLSearchParams): void {
    ...
    if (query.get("progress") !== "1") {
        sendJson(response, 200, buildLayer1View(projectFolder, repoDir, ref));
        return;
    }
    streamLayer1View(response, projectFolder, repoDir, ref);
}
