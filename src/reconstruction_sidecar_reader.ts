// The on-disk side of the file-history sidecar: the default blob reader, the default history root, the item-46 root-resolution chain (override → transcript-derived sibling → default), and the session-id scan. Split out of reconstruction_sidecar.ts (split, never condense) to keep that module — the pure backup→event transforms — within the 250-line cap. The `BackupReader` type itself stays in reconstruction_sidecar.ts (the transforms own it); this module imports it one-way.
...
// The file-history dir sitting next to a projects folder (<X>/projects → <X>/file-history), or undefined when no such sibling exists on disk (item 46: a tree copied out of ~/.claude mirrors its layout, so the blobs travel as the projects folder's sibling).
...
// Derive the sibling root from where the records' transcript actually sits on disk: <X>/projects/<project>/y.jsonl → <X>/file-history. A strict generalization of the default — a live ~/.claude/projects/<p>/x.jsonl transcript lands exactly on ~/.claude/file-history. undefined when no record has a source or no sibling dir exists.
...
// The file-history root resolution chain (item 46): explicit override → transcript-derived sibling → ~/.claude/file-history default.
...
// The session id from the transcript's envelope records (the file-history dir is named for it). file-history-snapshot records carry none, so scan for the first that has one.
...
// The distinct session ids across the records, in first-seen order. A merged multi-session transcript carries several; each session's backups live under its OWN file-history dir, so the reader must know all of them.
...
// The source entry whose projectsDir is the given transcript-derived projects root, by normalized-path equality. undefined when no declared source matches (that session falls back to the single-root chain). Exported for the multi-source root resolution (spec S5a), which matches sessions to sources the same way.
...
// The file-history root for one source-stamped record (spec S4a chain: the matching declared source's explicit fileHistoryDir → the record's transcript-derived sibling → the ~/.claude default).
...
// Per-session file-history roots for multi-source records (spec S4a, design §c5): each session's blobs live under the root of the source that recorded it, resolved from the session's first source-stamped record. Keyed by sessionId string (module-private internal map; the public reader surface still speaks Uuid — coding-req §1).
...
export function buildSidecarReader(records: TranscriptRecord[], sources?: SourceEntry[]): BackupReader | undefined {
    ...
    // spec S4a: with declared sources, each session reads from its OWN source's root; sessions no source claims keep the single-root chain above.
    const sessionRoots = sources === undefined ? undefined : computeSessionFileHistoryRoots(records, sources);
    // task 191: close out the CLI's "building sidecar backup reader" stage — the build itself is instant (blob reads happen lazily), so a hang after this line belongs to the next stage.
    ...
    return (backupFileName, sessionId) => {
        ...
        // The engine passes the snapshot's OWNING session: across merged sessions the same `@vN` blob name recurs with different content, so we MUST read the owner's copy. Fall back to a first-existing search only when the owner is unknown (a pre-sessionId caller).
