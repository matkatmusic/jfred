...
// Logs each build stage to stdout so tailing the server shows progress.
...
// GET /api/document — returns built document or streams NDJSON progress.
export function handleDocumentRequest(response: ServerResponse, query: URLSearchParams): void {
    ...
    // task 194: bounded mode — stop at boundFile's Nth revision.
    ...
    // item 46: the project's path overrides apply to everything below (both branches). Runs before any header goes out, so a malformed reveng-paths.json still 400s loudly.
    ...
    // Non-progress path: resolve + consent-decide + respond, all BEFORE any header, so a resolver refusal (bad project, traversal) still becomes a 400 via the outer catch. Consent-required is HTTP 200 with the kind discriminant (not 428) so browsers don't log the expected flow as an error.
    ...
    // Progress stream: open it now so the pre-build work is visible. Headers are already out, so any failure here is the terminal error line, not a 400 (a client-facing viewer request is always progress=1; a bad project already 400'd above at requireParam). The final line is the same object the non-progress path sends — the client keys off the `kind` discriminant.
    ...
    const writeNdjsonLine = (value: unknown): void => {
        // The synchronous build never yields to the event loop, so a client-abort 'close' event is never delivered mid-build. Detection works anyway: the first write after the client's RST fails synchronously inside net.Socket (uv_try_write EPIPE), which flips socket.writable to false without needing the loop — `destroyed` stays false until the loop turns, so it is the writable check that fires; the NEXT call here sees it and aborts the build.
        ...
        // res.write corks the socket and uncorks on nextTick — which never runs during the synchronous build, so every line would sit buffered until the build ends. Uncork NOW so each line flushes to the wire as it is written.
    ...
    try {
        ...
        // This parse is the only one (the build below reuses these records from the cache), so it streams the full per-record detail the console shows during a cold load.
        ...
        // The stringify below blocks the event loop for the whole document (seconds for a large project); announce it FIRST so the client shows "serializing document" instead of freezing on the last build line. The byte size is only known AFTER stringify, so the transfer label follows it. (item 82)
...
export function handleDiffRequest(response: ServerResponse, query: URLSearchParams): void {
    ...
    const allowScripts = query.get("allowScripts") === "1";
    // task 56: no question gate here — the timeline already decided; the param only keeps this request on the same cache entry as the document it is diffing.
    const reconstructPreBaseline = query.get("preBaseline") !== "0";
    // item 75: "Show full contents" — the revision-diff branch below widens git's context to the whole file when the client asks for it.
    const fullContext = query.get("context") === "full";
    // Untargeted on purpose: both diff views send no jsonl param, so this reuses the very project-wide artifact the views already built (equality certified by test_revision_diff_from_untargeted_document_matches_targeted_build).
...
// GET /api/range-patch — one git-apply-able unified diff for a picked contiguous step range over the WHOLE project's unified document. Consent mirrors /api/document's non-progress contract: consent-required is HTTP 200 with the kind discriminant (never a non-2xx), declined=1 builds the degraded document the client is already looking at.
...
// GET /api/step-files — the { path: content } map of every file present at one 1-based step, resolved on demand from the compact histories (skeleton steps carry no file map). One step's map is one repo snapshot — bounded, not multiplied. Consent mirrors /api/range-patch's non-progress contract.
