// Human-readable verdict trace over a LinePartition (s37 script-replay diagnostic, Phase A). Renders one
// row per line in original order — `<lineNumber>: <verdict>` — filtered by hideIgnored / onlyIgnored and
// optionally enriched (type + kind + preview, or the full pretty-printed record) under `--details`. Split
// out of reconstruction_parse_lines.ts to keep both files within the 250-line cap. Design:
// ~/.claude/plans/task-implement-script-replay-partitioned-puppy.md (Phase A, A4).
...
// Whether a row passes the kept/ignored filter. hideIgnored drops ignored rows; onlyIgnored drops kept
// rows (the two are inverses).
...
// A kind + preview for one content block: a tool_use shows its name + command (or file basename); a text block
// shows its text. undefined when the block carries neither.
...
// A kind + preview for a user record's tool result, inferred from its shape: a Read result nests file content,
// an Edit result carries before/after strings, a Write result carries content. Preview is the file basename.
...
// The tool/attachment kind + content preview of a parsed record, dispatched by shape: an attachment (e.g.
// edited_text_file), a file-history-snapshot, a tool_use / text content block, or a tool result on a user record.
...
// The record `type`, tool/attachment `kind`, a short preview, and the pretty-printed record of a raw line
// — read from the plain JSON (not the hydrated record) so `full` can pretty-print and an odd line never throws.
...
// The record `type` of a raw line, read on its own (cheaper than describeRaw — no pretty-print). `?` when
// the line is not parseable JSON or carries no string `type`.
...
// Render one row. A `--details`-selected row is enriched (previewOnly: type + kind + preview; full: type +
// kind then the pretty-printed record). A bare ignored row still surfaces its record `type:` — kept rows
// already name their type via the verdict, so this gives drops the same at-a-glance "what was this line".
...
// The single line a `byLine` detail selector restricts the output to (`--line N` / `--details N`), or
// undefined for an `all` / `byClass` selector (which render every row). A line selector names one specific
// line, so the trace shows only that line — detailed — rather than enriching it amid every other row.
...
// Render one row per line in original order, filtered by hideIgnored / onlyIgnored (mutually exclusive) and
// enriched by an optional `details` selector. A `byLine` selector also restricts the output to that one line.
