// Same-turn trunk absorption (task 145), split from reconstruction_trunk.ts for the line cap.  Parallel tool calls split ONE assistant API response into sibling JSONL records that share message.id; each tool_result then parents onto ITS OWN tool_use sibling, so the surviving ancestor walk reaches only one sibling's chain and the others dead-end (s85's `Write two.py` record and its result). Those records are the same live turn, not a rewind — the trunk absorbs them: (1) any assistant record sharing a trunk assistant record's message.id; (2) then, to a fixed point, any tool_result-only user record or attachment record whose parent is already absorbed.  A real rewound branch can never match: its records belong to a different API response (a new message.id) and its first diverging record is a prompt/assistant turn, not a bare tool_result.

import type { TranscriptRecord } from "./structures/envelope.ts";
import { BlockType, RecordType } from "./structures/vocabulary.ts";

// The message.id of an assistant record's API response — shared across every sibling record a multi-block response was split into. undefined for records with no message id.
...
// True when the user record's content is entirely tool_result blocks — a bare result echo of some tool_use, carrying no prompt of its own.
...
// True when the record is plumbing: an attachment or a bare tool_result echo. Anything else (a user prompt, an assistant reply) is real conversational content.
...
// True when the record is the same-turn tail of an absorbed chain: plumbing parented on a record the trunk already holds, with NOTHING but plumbing below it. A dead-end tool_result followed by real content (a hook-rewrite fork the user rewound after an assistant reply) is a genuine rewound exchange and must stay off the trunk so it still dims.
...
// Rule (1): absorb every assistant record that shares a message.id with a trunk assistant record — the split siblings of an API response the trunk already holds part of.
...
export function absorbParallelToolCallSiblings(records: TranscriptRecord[], trunk: Set<string>): void {
    ...
    // Rule (2): the absorbed siblings' result echoes and attachments, to a fixed point — an absorbed record may be the parent the next tail needs, so sweep until stable.
