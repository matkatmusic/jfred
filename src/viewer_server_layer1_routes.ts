...
//
// Split out of viewer_server.ts, which sat exactly at the repo's 250-line cap when tasks 292, 295,
// 296 and 297 added three more endpoints. The page's routes are the natural seam: they share a
// prefix and a page, so one lookup here is one branch there and the next Layer 1 endpoint costs
// viewer_server.ts nothing (precedent: viewer_api_sources.ts, split off viewer_server_routes.ts for
// the same cap).
//
// Returns whether it OWNED the request, so the caller's chain falls through unchanged when it
// did not.
