// GET + POST /api/layer1-settings — the per-project Layer 1 header record (task 297): the project
// folder, its repo, the ref, and the two picked source lists, so re-opening the page restores the
// last project instead of re-typing it.
//
// The file lives in the user's HOME, never in the repo: a settings file is per-user state and must
// not turn up as a git diff.
...
// One project's saved header state. Plain strings rather than Path/Date: this IS the on-disk record
// and the POST body, and it round-trips through JSON untouched.
...
// A settings file is a convenience, so a missing OR corrupt one reads as "nothing saved" rather
// than stopping the page from loading.
...
// The saved file PLUS the cold-start defaults, in one round trip: the page needs both to decide
// what to fill its boxes with, and asking twice would let it render once against the wrong answer.
...
// POST body = one project entry. Body accumulation follows handleConfigUpdate in viewer_server.ts,
// the existing precedent for a POST in this server.
