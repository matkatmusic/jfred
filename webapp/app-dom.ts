...
// `undefined` is permitted so a caller can pass an OPTIONAL attribute inline (e.g. a hover `title` that only some nodes have) instead of branching on it; el() omits those keys entirely.
...
// The element with `id`, thrown on absence so a markup drift fails loudly (shared by the layered and debug pages — task 183 moved it here from layered-app.ts).
...
export function el(tag: string, attrs: ElAttrs = {}, children: (Node | string)[] = []): HTMLElement {
    ...
    for (const [key, value] of Object.entries(attrs)) {
        // An absent optional attribute is omitted, not stringified: setAttribute would otherwise write the literal text "undefined" (e.g. an optional `title` on a Layer 1 node label).
