...
// Where one instant sits on the shared ruler. An instant the server did not resolve reads as the
// origin rather than NaN — the map is built from the same graph, so a miss means "no evidence".
function lookupAxisPx(instant: Date, offsets: AxisOffsetsPx): number {
    return offsets[instant.toISOString()] ?? 0;
}

// Pixels from `origin` to `instant` on that ruler — the ONE number that crosses into CSS, which
// still owns every placement rule. Either end missing means "no known offset", pinning to origin.
...
// where S5 found two distinct sessions observing the same bytes. Same `--axis-px` contract as a
// node dot, so line and dots share one ruler.
...
// per-session lanes inside (spec S8). JS emits only the server's finished ruler pixels —
// `--axis-px` for the widget's own offset and `--axis-span-px` for its height; layered-styles.css
// still owns every placement rule, so there is no JS layout pass.
