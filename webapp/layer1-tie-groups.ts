...
//
// A commit and a file's on-disk mtime routinely land on the same instant. Task 251 stopped them
// overprinting by charging that instant one stacked 22 px row per tied node — but once they are on
// separate rows, nothing says they were simultaneous, and the fix for the collision destroyed the
// information. This marker restores it, anchored on the single ruler tick that instant occupies.
//
// Its own module rather than more of webapp/layer1-page.ts, which is at the repo's 250-line cap —
// the split pattern layer1-sources.ts / layer1-find-file.ts / layer1-zoom.ts already set.
...
// Runs of two or more ladder nodes sharing one instant. ONE linear pass is correct because a tie is
// contiguous by construction: src/viewer_api_layer1.ts's listPairNodeLadder emits commits
// oldest-first then the on-disk node, and the axis charges tied nodes consecutive rows in that same
// order — so two nodes at one instant can never be separated by a node at another.
...
// The markers for one widget's ladder, positioned against that widget's own anchor (`startPx`, its
// earliest node) — the same widget-relative arithmetic layer1-page.ts does for the nodes themselves.
// `--span-px` is first-node-to-last-node, so the stylesheet adds the node box's own reach and needs
// no knowledge of how many rows the run holds.
