// Task 292: the JSONLs pane, below the File Nav. The folder tree's selection rules applied to
// session transcripts — a plain click picks one, a plain click on the only picked one clears it,
// shift extends (plans/layer1-mockup.html:1245-1271).
//
// What it filters is the FILE LIST: each surviving file keeps its WHOLE history, and the band
// webapp/layer1-ranges.ts draws over the canvas is what says which stretch of that history the
// session is answerable for. That is why this module reports targets and never touches the stage.
...
// Keyed by `fullPath`, not by the object: the list is re-fetched on every load, so an identity Set
// would silently lose its selection the moment the payload was replaced.
const selectedPaths = new Set<string>();
// Every path the drawn view knows about — the pairs plus both orphan buckets — as the view spells
// them: RELATIVE to the project folder. Empty until a view has been drawn, which reads as "nothing
// known yet" and shows every session rather than none.
...
// project folder. Translating here is what lets the two be compared at all — and dropping the
// paths outside the project folder is deliberate: a session that only touched files elsewhere has
// nothing to say about this timeline.
...
// A session earns a row only if it touched at least one file this view draws (user, 2026-07-27):
// the pane lists the transcripts answerable for THIS project, not every transcript under the
// source folders.
...
// The union of every picked session's touched paths, in the view's own spelling so
// filterLayer1ViewByTargets can match them. Empty means "not filtering", which is exactly what that
// filter reads an empty target list as.
...
// Fetch the sessions under the picked JSONL source folders. No source folders means no fetch and no
// sessions. A refusal is reported INTO the pane and nowhere else: this pane is an accessory, never
// a reason the timeline cannot draw.
...
// A plain click picks one; a plain click on the ONLY picked one clears it; shift toggles this one
...
function buildSessionItem(session: WireSession, onChange: () => void): HTMLElement {
    const item = el("div", { class: `session-item${selectedPaths.has(session.fullPath) ? " selected" : ""}` }, [
        ...
        // The day, then how much of the project this session touched — the two readings that decide
        // whether it is worth clicking, in the width the pane has.
...
// Draw the pane and wire its clear button. `onChange` is the page's re-filter — passed in for the
// same no-cycle reason wireFolderPickers takes `afterPick`.
export function renderSessionPane(onChange: () => void): void {
    ...
    // Property assignment, not addEventListener: this runs again on every load and on every click,
    // and a second listener would fire the previous render's callback as well.
    clear.onclick = () => {
        selectedPaths.clear();
        redraw();
    };
