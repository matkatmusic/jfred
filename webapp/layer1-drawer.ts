...
export function wireNodeDrawer(): void {
    getRequiredElementById("stage").addEventListener("click", (event) => {
        ...
        // Every n-commit node and every n-disk node opens the drawer: Layer 1 draws exactly ONE on-disk node per pair, so the task's "only the LATEST disk node is clickable" caveat is about a created-at node this layer does not draw — there is nothing to gate here yet.
        if (node === undefined) {
            return;
        }
