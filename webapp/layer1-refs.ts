// Task 286: the Layer 1 header's branch and commit dropdowns. They exist only once the repo box is
// confirmed to name a real git repo — GET /api/layer1-refs answering ok IS that confirmation, so no
// separate "is this a repo" endpoint exists. Both selects WRITE into the pre-existing #ref text box
// rather than replacing it: a raw hash must still be typeable, and #ref is the endpoint's param.

import { el, getInputById, getRequiredElementById } from "./app-dom.ts";

// The /api/layer1-refs wire shape, re-declared because webapp/ cannot import from src/ — the same
// constraint that re-spells CommitTimeSource in layer1-sources.ts.
...
// The row the `.repo-ok` class gates the pickers on. Thrown on absence for the same reason
// getRequiredElementById throws: markup drift should fail loudly, not silently do nothing.
...
// Mockup line 1207's expression: short hash, padded subject, date — monospace, so the columns line
// up down the list.
...
function fillBranchOptions(refs: WireLayer1Refs, ref: string): void {
    ...
    // The ref box wins when it names a branch — that is what the view on screen was built against;
    // otherwise the branch the repo is actually checked out on.
...
function fillCommitOptions(commits: WireRefCommit[], ref: string): void {
    ...
    // Only select a hash the list actually holds: assigning an unmatched value blanks the select,
    // which reads worse than "(branch head)".
    select.value = commits.some((commit) => commit.hash === ref) ? ref : "";
}

// Read #repo, ask the endpoint for its branches and head-window of commits, and reveal the pickers.
// Any refusal — empty box, a path that is not a repo, a bad ref — just removes `.repo-ok`, so the
// dropdowns silently do not appear (the mockup's behaviour). The crumb already carries the view
// route's own errors; a second error surface here would double-report the same bad path.
...
// `afterBranchPick` is passed in rather than imported, matching wireFolderPickers: its only caller
// is layer1-page.ts's boot, and reaching back for loadLayer1View would make the modules circular.
export function wireRefPickers(afterBranchPick: () => void): void {
    getSelectById("branch").addEventListener("change", (event) => {
        ...
        // The user's decision: picking a branch reloads the whole view. The reload re-runs
        // confirmRepoAndFillRefs, so the commit list becomes that branch's with "(branch head)" on.
        afterBranchPick();
    });
    getSelectById("commit").addEventListener("change", (event) => {
        // Writes the hash and stops. No reload: a commit pick is a ~10 s rebuild, and a select fires
        // `change` on every keyboard arrow-through — the user presses Load when they have landed.
        getInputById("ref").value = (event.currentTarget as HTMLSelectElement).value;
    });
