// Commit-node and tool-call-node derivation (split from timeline-nodes.ts, 250-line cap): the wire document's gitOperations and toolCalls become their unnumbered timeline rows.
...
// Commit pick hard-stops: from the document's commit operations (which carry the message) when it ships gitOperations; an older cached document lacks the field and falls back to commitMarkers.
...
// One un-bubbled row per document tool call (item 55); the sort interleaves them chronologically with the turns they ran between. A call whose record ran a FAILED git command is stamped isError (task 103: joined by record uuid to the errored gitOperations — only git rows badge, not every failed tool call; hook-rewrite rows carry the attachment record's uuid and never match).
export function deriveToolCallNodes(document: WireTimelineDocument): ToolCallNode[] {
    ...
    // task 67: only runs the sandbox proved modified files ride their rows — a read-only run (empty changedPaths) behaves like any other tool call.
