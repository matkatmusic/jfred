// task 56 follow-up: the git-derived baseline pane — the unrecoverable-placeholder pattern (task-126 banner stack) applied to base-commit beacon revisions. A beacon's computed diff is usually empty ("no content change in this revision"), so the pane shows the committed bytes themselves: a banner naming the base commit, a provenance note, then the revision's full content, syntax-highlighted.
...
export function showGitBaselineInDetails(historyDocument: WireHistoryDocument, path: string, changeId: string): void {
    // The document's file histories carry each revision's own lines, so the committed content is derivable client-side — no fetch.
    ...
    // showContentInDetails owns the pane lifecycle (label, toggle reset, highlight); the banner stack is prepended after, mirroring the task-126 unrecoverable pane.
