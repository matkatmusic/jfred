// File-change derivation for the revision timeline (split from timeline.ts, task 92): resolving step changeIds through the revision index into displayable chips, plus patch splitting and the per-node chip stamping pass.
...
// Wire-string mirror of BASE_COMMIT_CHANGE_ID_PREFIX (src/reconstruction_base_commit.ts) — a changeId with this prefix is a base-commit baseline beacon, evidenced by no session's record.
export const GIT_BASE_CHANGE_ID_PREFIX = "gitBase:";

// Wire-string mirror of SCRIPT_RUN_CHANGE_ID_PREFIX (src/reconstruction_script_execution.ts) — a `scriptRun:<tool_use id>:<target>` changeId marks a synthetic script-execution revision (task 67: the Details pane joins a run's changed files to these revisions).
export const SCRIPT_RUN_CHANGE_ID_PREFIX = "scriptRun:";

// True when every changeId on the step is a base-commit beacon (task 86): the step carries the repo's pre-session state, evidenced by no session's record.
...
// The commit hash inside gitBase:<hash>:<target> — the second colon-separated field (hashes never contain colons); "" for a malformed id, never a throw. (Moved from details-model.ts, task 121 — the gitBase vocabulary's canonical home.)
...
// Entry-time path per revision (task 127): walk backward from the final target, stepping each rename's `from` across it, so revisions before a rename display the name the file had at that time.
...
// changeId -> { path, eventKind, renamedFrom, isRewound } across surviving AND rewound histories, so a step's changeIds resolve to displayable file chips and orphan detection in one lookup.  Surviving histories are indexed first and win duplicates (a changeId present in both branches counts as surviving).
...
// A step's displayable file chips: each changeId resolved through the revision index, deduped by path; changeIds that resolve nowhere fall back to the step's changedPaths hint (kind: edit).
...
// The file route a chip's revision jumps to ("#/project/<p>/file/<path>/rev/<n>", which lands in THE Revision View since task 93), or undefined when the change carries no changeId, the changeId is not a backup blob name (task 94: only revisions actually backed by a File History Snapshot get the 📷 button — tool-evidenced `toolu_…` ids resolve to revisions too, but have no snapshot), or it resolves to no surviving revision number (re-stamped synthetic ids, blob names without an anchored revision) — those chips get no jump button rather than a dead link.
...
// old (pre engine-stamped isOrphaned): checkStepIsOrphaned, the per-step snapshot proxy — retired; preserved in archive/timeline-changes-retired-orphan-proxies.ts (task 121).

// Split a multi-file range patch on its `diff --git ` headers into per-file blocks, each keyed by its patch-relative b/ path (the range-diff inspector shows one file's block at a time).
...
// task 133: the merge dedupe key. Two snapshots on one turn may hold DISTINCT revisions of the same file (baseline-demo: an external user-edit then an agent Edit) — each keeps its chip; only a repeat of the SAME revision collapses. Fallback chips (no changeId) still collapse per path.
function computeMergedChipKey(change: FileChange): string {
    return `${change.path}|${change.changeId ?? ""}`;
}

// A turn's file chips: deriveFileChanges merged over its snapshots, deduped by path+changeId (task 133; deriveFileChanges keeps its own within-step path dedupe — rename-pair collapse lives there).
...
// old (pre engine-stamped isOrphaned): checkTurnIsOrphaned, the snapshot-proxy orphan check — retired; preserved in archive/timeline-changes-retired-orphan-proxies.ts (task 121).

// fileChanges on every snapshot-owning node (user turns and session ends own empty snapshot lists, so they resolve to no chips). Tool-call nodes never own snapshots; plain commit rows own none either — but the merged baseline commit row does, and gets its chips (task 121).
