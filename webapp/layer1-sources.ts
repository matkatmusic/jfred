...
// Which git stamp places a bubble (task 282). These bare strings mirror `CommitTimeSource` in
// src/structures/vocabulary.ts, re-spelled because webapp/ cannot import from src/ — the same
// constraint that re-declares `Instant` atop layer1-ruler-axis.ts.
...
export function readSourceParams(): URLSearchParams {
    ...
    // Only the non-default reading is written, so a default view's URL — and every link already
    // shared — stays exactly as it was.
...
export function fillSourceBoxesFromUrl(): void {
    ...
    // An unknown ?time= is ignored rather than trusted: it would otherwise reach the endpoint as
    // a stamp name no git command understands.
